        ...
        @if ($products->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                @foreach ($products as $product)
                    <article class="border border-gray-200 dark:border-gray-700 rounded-xl p-6 hover:shadow-md transition-shadow">
                        ...
                        <p class="text-gray-600 dark:text-gray-400 mb-4">
                            {{ \Illuminate\Support\Str::limit($product->getTranslated('short_description', app()->getLocale()) ?? strip_tags($product->getTranslated('long_description', app()->getLocale())), 150) }}
                        </p>

                        <dl class="text-sm text-gray-500 dark:text-gray-400 mb-4">
                            <div class="flex justify-between">
                                <dt>{{ __('shop::front.general.category') }}</dt>
                                <dd>{{ optional($product->category)->getTranslated('title', app()->getLocale()) ?? '—' }}</dd>
                            </div>
                            <div class="flex justify-between">
                        ...
                        <a href="{{ Modules\Shop\App\Http\Controllers\Front\ShopController::resolveProductUrl($product) }}"
                           class="inline-flex items-center text-blue-600 dark:text-blue-400 hover:underline">
                            ...
                            <svg class="ml-2 h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                                <path d="M12.293 4.293a1 1 0 011.414 0L18 8.586a1 1 0 010 1.414l-4.293 4.293a1 1 0 01-1.414-1.414L14.586 10H4a1 1 0 110-2h10.586l-2.293-2.293a1 1 0 010-1.414z"/>
                            </svg>
                        </a>
                    </article>
                @endforeach
