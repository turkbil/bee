...
namespace Modules\Service\App\Http\Livewire\Traits;
 ...
 *
 * Service modülü için queue-based bulk işlemler:
 * - Bulk delete (queue)
 * - Bulk update (queue)
 * - Progress tracking
...
trait WithBulkActionsQueue
{
    ...
    public function bulkDelete()
    {
        ...
        try {
            ...
            \Modules\Service\App\Jobs\BulkDeleteServicesJob::dispatch(
                $this->selectedItems,
                $tenantId,
                $userId,
                ['force_delete' => false]
            ...
            $this->bulkProgress = [
                'operation' => 'delete',
                'count' => $count,
                'progress_key' => "bulk_delete_services_{$tenantId}_{$userId}",
    ...
    public function bulkUpdate($updateData)
    {
        ...
        try {
            ...
            \Modules\Service\App\Jobs\BulkUpdateServicesJob::dispatch(
                $this->selectedItems,
                $updateData,
                $tenantId,
                $userId,
            ...
            $this->bulkProgress = [
                'operation' => 'update',
                'count' => $count,
                'progress_key' => "bulk_update_services_{$tenantId}_{$userId}",
                'started_at' => now()->toISOString()
            ];
     ...
     */
    public function translateContent($data, ?int $serviceId = null): void
    {
        if (!$serviceId) {
            $this->dispatch('toast', [
                'title' => 'Çeviri Hatası',
        ...
        try {
            ...
            $progressKey = "service_translation_progress_{$serviceId}_" . uniqid();

            // Translation job dispatch et
            $job = \Modules\Service\App\Jobs\TranslateServiceContentJob::dispatch($data, $serviceId);
