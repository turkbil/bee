...
class MuzibuCategoryObserver
{
    ...
    /**
     * Handle the MuzibuCategory "creating" event.
     * Yeni kayıt oluşturulmadan önce çalışır
     */
    public function creating(MuzibuCategory $category): void
    {
        ...
        Log::info('Muzibu Category creating', [
    ...
    /**
     * Handle the MuzibuCategory "created" event.
     * Kayıt oluşturulduktan sonra çalışır
     */
    public function created(MuzibuCategory $category): void
    {
        // Cache temizle
        ...
        Log::info('Muzibu Category created successfully', [
    ...
    /**
     * Handle the MuzibuCategory "updating" event.
     * Güncelleme yapılmadan önce çalışır
     */
    public function updating(MuzibuCategory $category): void
    {
        ...
        Log::info('Muzibu Category updating', [
            'category_id' => $category->category_id,
    ...
    /**
     * Handle the MuzibuCategory "updated" event.
     * Güncelleme yapıldıktan sonra çalışır
     */
    public function updated(MuzibuCategory $category): void
    {
        // Cache temizle
        ...
        Log::info('Muzibu Category updated successfully', [
            'category_id' => $category->category_id,
            'user_id' => auth()->id()
        ]);
    }
    ...
    /**
     * Handle the MuzibuCategory "saving" event.
     * Create veya Update'ten önce çalışır (her ikisinde de)
     */
    public function saving(MuzibuCategory $category): void
    {
        // Title validasyon
            ...
            foreach ($category->title as $locale => $title) {
                ...
                if (!empty($title)) {
                    ...
                    if (strlen($title) > $maxLength) {
                        ...
                        Log::warning('Muzibu Category title auto-trimmed', [
                            'category_id' => $category->category_id,
                            'locale' => $locale,
    ...
    /**
     * Handle the MuzibuCategory "saved" event.
     * Create veya Update'ten sonra çalışır (her ikisinde de)
     */
    public function saved(MuzibuCategory $category): void
    {
        // Universal SEO cache temizle
        Cache::forget("universal_seo_muzibu_category_{$category->category_id}");

        // Response cache temizle
        if (function_exists('responsecache')) {
            responsecache()->forget(route('muzibu.category.show', $category->slug));
        }
    }

    /**
     * Handle the MuzibuCategory "deleting" event.
     * Silme işleminden önce çalışır
     */
    public function deleting(MuzibuCategory $category): bool
    {
        // Kategoriye bağlı muzibular varsa silme
        if ($category->muzibus()->count() > 0) {
            throw new \Exception('Bu kategoriye ait muzibular var. Önce muzibuları silmelisiniz.');
        }

        Log::info('Muzibu Category deleting', [
            'category_id' => $category->category_id,
            'title' => $category->title,
            'user_id' => auth()->id()
        ]);
    ...
    /**
     * Handle the MuzibuCategory "deleted" event.
     * Silme işleminden sonra çalışır
     */
    public function deleted(MuzibuCategory $category): void
    {
        // Cache temizle
        $this->clearCategoryCaches($category->category_id);
    ...
    /**
     * Handle the MuzibuCategory "restoring" event.
     * Soft delete'ten geri dönüşte çalışır
     */
    public function restoring(MuzibuCategory $category): void
    {
        Log::info('Muzibu Category restoring', [
            'category_id' => $category->category_id,
            'title' => $category->title,
            'user_id' => auth()->id()
        ]);
    ...
    /**
     * Handle the MuzibuCategory "restored" event.
     * Soft delete'ten geri döndükten sonra çalışır
     */
    public function restored(MuzibuCategory $category): void
    {
        // Cache temizle
        $this->clearCategoryCaches();
        ...
        Log::info('Muzibu Category restored successfully', [
            'category_id' => $category->category_id,
            'title' => $category->title,
            'user_id' => auth()->id()
        ]);
    ...
    /**
     * Handle the MuzibuCategory "forceDeleting" event.
     * Kalıcı silme işleminden önce çalışır
     */
    public function forceDeleting(MuzibuCategory $category): bool
    {
        Log::warning('Muzibu Category force deleting', [
            'category_id' => $category->category_id,
            'title' => $category->title,
            'user_id' => auth()->id()
        ]);
    ...
    /**
     * Handle the MuzibuCategory "forceDeleted" event.
     * Kalıcı silme işleminden sonra çalışır
     */
    public function forceDeleted(MuzibuCategory $category): void
    {
    ...
    /**
     * Muzibu Category cache'lerini temizle
     */
    private function clearCategoryCaches(?int $categoryId = null): void
    {
        // TenantCacheService ile prefix bazlı temizleme
        $this->cacheService->flushByPrefix('muzibu_categories');

        // Spesifik cache key'leri temizle
        Cache::forget('muzibu_categories_list');
        Cache::forget('muzibu_categories_menu_cache');

        if ($categoryId) {
            Cache::forget("muzibu_category_detail_{$categoryId}");
            Cache::forget("universal_seo_muzibu_category_{$categoryId}");
        }

        // Tag bazlı cache temizleme
        if (Cache::getStore() instanceof \Illuminate\Cache\TaggableStore) {
            Cache::tags(['muzibu_categories', 'content'])->flush();
        }
    ...
    {
        $query = MuzibuCategory::whereRaw("JSON_UNQUOTE(JSON_EXTRACT(slug, '$.{$locale}')) = ?", [$slug]);
