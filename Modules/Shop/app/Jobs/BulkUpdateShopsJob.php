...
/**
 * ✏️ Bulk Shop Update Queue Job
 *
 * Shop modülünün bulk güncelleme işlemleri için queue job:
 ...
 */
class BulkUpdateShopsJob implements ShouldQueue
{
    ...
    /**
     * @param array $shopIds Güncellenecek sayfa ID'leri
    ...
    public function handle(ShopService $shopService): void
    {
        ...
        try {
            Log::info('✏️ BULK SHOP UPDATE STARTED', [
            ...
            $progressKey = "bulk_update_shops_{$this->tenantId}_{$this->userId}";
            ...
            foreach ($this->shopIds as $index => $shopId) {
                try {
                    ...
                    if ($this->options['validate'] ?? true) {
                        $this->validateUpdateData($shop, $filteredUpdateData);
                    }

                    // Güncelleme işlemi
                    $shop->update($filteredUpdateData);
                    log_activity($shop, 'toplu-güncellendi');
                    ...
                    $errorCount++;
                    $errors[] = "Sayfa güncelleme hatası (ID: {$shopId}): " . $e->getMessage();
            ...
            Log::info('✅ BULK SHOP UPDATE COMPLETED', [
            ...
            Log::error('💥 BULK SHOP UPDATE FAILED', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
     ...
     */
    private function validateUpdateData(Shop $shop, array $updateData): void
    {
        // Slug benzersizlik kontrolü
        if (isset($updateData['slug'])) {
            $existingPage = Shop::where('slug', $updateData['slug'])
                ->where('id', '!=', $shop->id)
                ->first();
    ...
    private function clearPageCaches(): void
    {
        try {
            ...
            Log::info('🗑️ Shop caches cleared after bulk update');
        } catch (\Exception $e) {
    ...
    {
        Log::error('💥 BULK SHOP UPDATE JOB FAILED', [
            'shop_ids' => $this->shopIds,
            'update_data' => $this->updateData,
            'tenant_id' => $this->tenantId,
            'user_id' => $this->userId,
