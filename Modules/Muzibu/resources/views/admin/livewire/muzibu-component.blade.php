...
<div class="muzibu-component-wrapper">
    <div class="card">
                ...
                <div class="col">
                    <div class="input-icon">
                        ...
                        <input type="text" wire:model.live="search" class="form-control"
                            placeholder="{{ __('muzibu::admin.search_placeholder') }}">
                    </div>
                </div>
                <!-- Ortadaki Loading -->
            ...
            <div id="table-default" class="table-responsive">
                <table class="table table-vcenter card-table table-hover text-nowrap datatable">
                    ...
                    <tbody class="table-tbody">
                        @forelse($muzibus as $muzibu)
                            <tr class="hover-trigger" wire:key="row-{{ $muzibu->muzibu_id }}">
                                <td class="sort-id small">
                                    <div class="hover-toggle">
                                        <span class="hover-hide">{{ $muzibu->muzibu_id }}</span>
                                        <input type="checkbox"
                                               wire:model.live="selectedItems"
                                               value="{{ $muzibu->muzibu_id }}"
                                               class="form-check-input hover-show"
                                               id="checkbox-{{ $muzibu->muzibu_id }}"
                                               @checked(in_array($muzibu->muzibu_id, $selectedItems))>
                                    </div>
                                </td>
                                <td wire:key="title-{{ $muzibu->muzibu_id }}" class="position-relative">
                                    ...
                                    @else
                                        <div class="d-flex align-items-center">
                                            <span
                                                class="editable-title pr-4">{{ $muzibu->getTranslated('title', $currentSiteLocale) ?? $muzibu->getTranslated('title', 'tr') }}</span>
                                            <button class="btn btn-sm px-2 py-1 edit-icon ms-4"
                                                wire:click="startEditingTitle({{ $muzibu->muzibu_id }}, '{{ addslashes($muzibu->getTranslated('title', $currentSiteLocale) ?? $muzibu->getTranslated('title', 'tr')) }}')">
                                ...
                                <td class="text-center align-middle">
                                    <button wire:click="toggleActive({{ $muzibu->muzibu_id }})"
                                        ...
                                        <div wire:loading.remove
                                            wire:target="toggleActive({{ $muzibu->muzibu_id }})">
                                            @if ($muzibu->is_active)
                                                <i class="fas fa-check"></i>
                                            @else
                                                <i class="fas fa-times"></i>
                                ...
                                <td class="text-center align-middle">
                                    <div class="d-flex align-items-center gap-3 justify-content-center">
                                        <a href="{{ route('admin.muzibu.manage', $muzibu->muzibu_id) }}"
                                        ...
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                <a href="javascript:void(0);"
                                                    ...
                                                    module: 'muzibu',
                                                    id: {{ $muzibu->muzibu_id }},
                                                    title: '{{ addslashes($muzibu->getTranslated('title', app()->getLocale()) ?? $muzibu->getTranslated('title', 'tr')) }}'
                                                })"
        ...
        <!-- Bulk Actions -->
        @include('muzibu::admin.partials.bulk-actions', ['moduleType' => 'muzibu'])
