...
class WSA121_Istif_2_Detailed extends Seeder {
    public function run(): void {
        $p = DB::table('shop_products')->where('sku', 'WSA121')->first();
        if (!$p) {$this->command->error('❌ Master bulunamadı: WSA121'); return; }
        DB::table('shop_products')->where('product_id', $p->product_id)->update([
            'long_description' => json_encode(['tr' => '
<section><h2>İXTİF WSA121: Ağır Hizmette Hız, Hassasiyet ve Li‑Ion Konforu</h2>
<p>Depo koridorları daralırken sipariş hacmi artıyor. WSA121, Li‑Ion teknolojisiyle tasarlanmış ağır hizmet
...
USB çıkışı günlük akışa pratiklik katar. 1200 kg nominal kapasite ile yoğun vardiyalarda stabil performans üretir.</p></section>
<section><h3>Teknik Güç ve Performans</h3>
<p>WSA121’de AC sürüş motoru ve güçlü kaldırma motoru birlikte çalışarak istif süresini kısaltır. Yüklü/boş
...
sayesinde palet üst seviye raflarda dahi sarsıntısız ve kontrollü hareket eder. Kompakt mimariyle kısa l2 = 714 mm
ve dar dönüş yarıçapı (Wa = 1446 mm) dar koridorlarda akıcı palet akışı sağlar. Direk kapalı yüksekliği h1 = 1970 mm,
nominal kaldırma yüksekliği h3 = 2915 mm ile dengeli ağırlık dağılımı ve görüş alanı hedeflenir. Elektromanyetik
frenleme, rampalarda ve platform kenarlarında güvenli duruş yaratır. Li‑Ion batarya paketi (24V/100Ah) vardiya
içinde mola şarjlarını mümkün kılar; bellek etkisi yoktur, bakım gerektirmez.</p></section>
<section><h3>Sonuç</h3>
<p>Yoğun sipariş rejiminde her saniye kıymetli. WSA121, hız ve hassasiyeti bir araya getirip iş güvenliğini
                ...
                [
                    {
                        ...
                        "answer": "Minimum dönüş yarıçapı 1446 mm değerini dikkate alarak koridor genişliği planlanmalıdır; palet ve güvenlik payı eklenmelidir."
                    },
                    {
...
JSON
        ...
        $this->command->info('✅ Detailed: WSA121 güncellendi');
    }
}
