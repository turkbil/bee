...
<div class="service-component-wrapper">
    <div class="card">
        @include('service::admin.helper')
                ...
                <div class="col">
                    <div class="input-icon">
                        ...
                        <input type="text" wire:model.live="search" class="form-control"
                            placeholder="{{ __('service::admin.search_placeholder') }}">
                    </div>
                </div>
                <!-- Ortadaki Loading -->
                ...
                <table class="table table-vcenter card-table table-hover text-nowrap datatable">
                            ...
                            <th style="width: 50px">
                                <div class="d-flex align-items-center gap-2">
                                    <input type="checkbox"
                                           ...
                                           @checked($selectAll)>
                                    <button
                                        class="table-sort {{ $sortField === 'service_id' ? ($sortDirection === 'asc' ? 'asc' : 'desc') : '' }}"
                                        wire:click="sortBy('service_id')">
                                    </button>
                                </div>
                            </th>
                            <th>
                    ...
                    <tbody class="table-tbody">
                        @forelse($services as $service)
                            <tr class="hover-trigger" wire:key="row-{{ $service->service_id }}">
                                <td class="sort-id small">
                                    <div class="hover-toggle">
                                        <span class="hover-hide">{{ $service->service_id }}</span>
                                        <input type="checkbox"
                                               wire:model.live="selectedItems"
                                               value="{{ $service->service_id }}"
                                               class="form-check-input hover-show"
                                               id="checkbox-{{ $service->service_id }}"
                                               @checked(in_array($service->service_id, $selectedItems))>
                                    </div>
                                </td>
                                <td wire:key="title-{{ $service->service_id }}" class="position-relative">
                                        ...
                                        <div class="d-flex align-items-center gap-3" x-data
                                            ...
                                            <div class="flexible-input-wrapper">
                                                <input type="text" wire:model.defer="newTitle"
                                                    ...
                                                    placeholder="{{ __('service::admin.title_field') }}"
                                    ...
                                    @else
                                        <div class="d-flex align-items-center">
                                            <span
                                                class="editable-title pr-4">{{ $service->getTranslated('title', $currentSiteLocale) ?? $service->getTranslated('title', 'tr') }}</span>
                                            <button class="btn btn-sm px-2 py-1 edit-icon ms-4"
                                                wire:click="startEditingTitle({{ $service->service_id }}, '{{ addslashes($service->getTranslated('title', $currentSiteLocale) ?? $service->getTranslated('title', 'tr')) }}')">
                                ...
                                <td class="text-center align-middle">
                                    <button wire:click="toggleActive({{ $service->service_id }})"
                                        class="btn btn-icon btn-sm {{ $service->is_active ? 'text-muted bg-transparent' : 'text-red bg-transparent' }}">
                                        <!-- Loading Durumu -->
                                        <div wire:loading wire:target="toggleActive({{ $service->service_id }})"
                                            class="spinner-border spinner-border-sm">
                                        </div>
                                        <!-- Normal Durum: Aktif/Pasif İkonları -->
                                        <div wire:loading.remove
                                            wire:target="toggleActive({{ $service->service_id }})">
                                ...
                                <td class="text-center align-middle">
                                    <div class="d-flex align-items-center gap-3 justify-content-center">
                                        <a href="{{ route('admin.service.manage', $service->service_id) }}"
                                            data-bs-toggle="tooltip" data-bs-placement="top"
                                            title="{{ __('admin.edit') }}"
                                            style="min-height: 24px; display: inline-flex; align-items: center; text-decoration: none;">
                                            <i class="fa-solid fa-pen-to-square link-secondary fa-lg"></i>
                                        </a>
                                        @if(config('service.integrations.studio.enabled', false) && class_exists(config('service.integrations.studio.component', '')))
                                        <a href="{{ route('admin.studio.editor', ['module' => 'service', 'id' => $service->service_id]) }}"
                                            target="_blank" data-bs-toggle="tooltip" data-bs-placement="top"
                                            title="{{ __('service::admin.studio.editor') }}"
                                            style="min-height: 24px; display: inline-flex; align-items: center; text-decoration: none;">
                                            <i class="fa-solid fa-wand-magic-sparkles link-secondary fa-lg"></i>
                                        </a>
                                        @endif
                                        <x-ai-translation :entity-type="'service'" :entity-id="$service->service_id"
                                            tooltip="{{ __('admin.ai_translate') }}" />
                                        @hasmoduleaccess('service', 'delete')
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                <a href="javascript:void(0);"
                                                    ...
                                                    title: '{{ addslashes($service->getTranslated('title', app()->getLocale()) ?? $service->getTranslated('title', 'tr')) }}'
                                                })"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-4">
                                    <div class="empty">
                                        <p class="empty-title">{{ __('service::admin.no_services_found') }}</p>
                                        <p class="empty-subtitle text-muted">
                                            {{ __('service::admin.no_results') }}
                                        </p>
                                    </div>
                                </td>
                            </tr>
            ...
            @else
                <div class="d-flex justify-content-between align-items-center mb-0">
                    <p class="small text-muted mb-0">
                        Toplam <span class="fw-semibold">{{ $services->total() }}</span> sonuç
                    </p>
                </div>
            @endif
        </div>
        ...
        <!-- Bulk Actions -->
        @include('service::admin.partials.bulk-actions', ['moduleType' => 'service'])
