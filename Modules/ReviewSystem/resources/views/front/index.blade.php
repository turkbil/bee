...
@section('module_content')
    <div class="bg-white dark:bg-gray-900" x-data="reviewsystemsIndex()" x-init="init()">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
                <div class="max-w-full">
                    <h1 class="text-4xl font-semibold text-gray-900 dark:text-white mb-4">
                        {{ __('reviewsystem::front.general.reviewsystems') }}
                    </h1>
                </div>
            </div>
        </div>
                            ...
                            <h2
                                ...
                                @php
                                    ...
                                    if (is_array($slugData)) {
                                        ...
                                        $slug = $item->getCurrentSlug() ?? $item->reviewsystem_id;
                                    }
                        ...
                        <div class="flex items-center space-x-2">
                            @if ($items->onFirstPage())
                                <span class="px-4 py-2 text-sm text-gray-400 dark:text-gray-600">
                                    {{ __('reviewsystem::front.general.previous') }}
                                </span>
                            @else
                            ...
                            @if ($items->hasMorePages())
                                <a href="{{ $items->nextPageUrl() }}"
                                    class="px-4 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white transition-colors">
                                    {{ __('reviewsystem::front.general.next') }}
                                </a>
                            @else
                                <span class="px-4 py-2 text-sm text-gray-400 dark:text-gray-600">
                                    {{ __('reviewsystem::front.general.next') }}
                ...
                <div class="text-center py-16">
                    <div class="max-w-md mx-auto">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 dark:text-white mb-2">
                            {{ __('reviewsystem::front.general.no_reviewsystems_found') }}
                        </h3>
                        <p class="text-gray-600 dark:text-gray-400">
                            {{ __('reviewsystem::front.general.no_reviewsystems_description') }}
                        </p>
                    </div>
                </div>
            @endif
