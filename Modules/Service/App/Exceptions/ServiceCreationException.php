<?php

namespace Modules\Service\App\Exceptions;

class ServiceCreationException extends ServiceException
{
    public function getErrorType(): string
    {
        return 'service_creation_failed';
    }

    public static function withValidationErrors(array $errors): self
    {
        return new self(
            message: 'Service creation failed due to validation errors',
            context: ['validation_errors' => $errors]
        );
    }

    public static function withDatabaseError(string $error): self
    {
        return new self(
            message: 'Service creation failed due to database error',
            context: ['database_error' => $error]
        );
    }
}
