...
class ServiceCategoryService
{
     ...
     */
    public function findCategoryWithSeo(int $id): ?ServiceCategory
    {
    ...
    public function createCategory(array $data): array
    {
        try {
            ...
            Log::info('Service Category created', [
            ...
            return [
                'success' => false,
                'message' => __('service::admin.category_create_failed'),
                'error' => $e->getMessage()
            ];
        }
    }
    ...
    public function updateCategory(int $id, array $data): bool
    {
        ...
        if ($result) {
            Log::info('Service Category updated', [
                'category_id' => $id,
                'user_id' => auth()->id()
            ]);
        }
    ...
    public function deleteCategory(int $id): array
    {
        try {
            ...
            if ($category->services()->count() > 0) {
                Log::warning('Cannot delete category with services', [
                    'category_id' => $id,
                ...
                return [
                    'success' => false,
                    'message' => __('service::admin.category_has_services')
                ];
            }
            ...
            if ($result) {
                Log::info('Service Category deleted', [
                    'category_id' => $id,
                    'user_id' => auth()->id()
                ]);
                ...
                return [
                    'success' => true,
                    'message' => __('service::admin.category_deleted')
                ];
            }

            return [
                'success' => false,
                'message' => __('service::admin.category_delete_failed')
            ];
        } catch (\Exception $e) {
            Log::error('Service Category deletion failed', [
                'category_id' => $id,
                'error' => $e->getMessage(),
                'user_id' => auth()->id()
            ]);
    ...
    public function toggleCategoryStatus(int $id): array
    {
        try {
            ...
            if ($result) {
                Log::info('Service Category status toggled', [
                    'category_id' => $id,
                ...
                return [
                    'success' => true,
                    'type' => 'success',
                    'message' => __('service::admin.status_updated'),
                    'data' => $category,
            ...
            return [
                'success' => false,
                'type' => 'error',
                'message' => __('service::admin.status_update_failed')
            ];
        } catch (\Exception $e) {
