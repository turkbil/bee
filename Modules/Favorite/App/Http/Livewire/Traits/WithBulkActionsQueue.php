...
namespace Modules\Favorite\App\Http\Livewire\Traits;
 ...
 *
 * Favorite modülü için queue-based bulk işlemler:
 * - Bulk delete (queue)
 * - Bulk update (queue)
 * - Progress tracking
...
trait WithBulkActionsQueue
{
    ...
    public function bulkDelete()
    {
        ...
        try {
            ...
            \Modules\Favorite\App\Jobs\BulkDeleteFavoritesJob::dispatch(
                $this->selectedItems,
                $tenantId,
                $userId,
                ['force_delete' => false]
            ...
            $this->bulkProgress = [
                'operation' => 'delete',
                'count' => $count,
                'progress_key' => "bulk_delete_favorites_{$tenantId}_{$userId}",
    ...
    public function bulkUpdate($updateData)
    {
        ...
        try {
            ...
            \Modules\Favorite\App\Jobs\BulkUpdateFavoritesJob::dispatch(
                $this->selectedItems,
                $updateData,
                $tenantId,
                $userId,
            ...
            $this->bulkProgress = [
                'operation' => 'update',
                'count' => $count,
                'progress_key' => "bulk_update_favorites_{$tenantId}_{$userId}",
                'started_at' => now()->toISOString()
            ];
     ...
     */
    public function translateContent($data, ?int $favoriteId = null): void
    {
        if (!$favoriteId) {
            $this->dispatch('toast', [
                'title' => 'Çeviri Hatası',
        ...
        try {
            ...
            $progressKey = "favorite_translation_progress_{$favoriteId}_" . uniqid();

            // Translation job dispatch et
            $job = \Modules\Favorite\App\Jobs\TranslateFavoriteContentJob::dispatch($data, $favoriteId);
