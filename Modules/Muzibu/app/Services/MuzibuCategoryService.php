...
class MuzibuCategoryService
{
    public function __construct(
        private readonly MuzibuCategoryRepository $repository
    ) {}
     ...
     * Find category by ID
     */
    public function findCategory(int $id): ?MuzibuCategory
    {
     ...
     */
    public function findCategoryWithSeo(int $id): ?MuzibuCategory
    {
    ...
    public function createCategory(array $data): array
    {
        try {
            ...
            Log::info('Muzibu Category created', [
            ...
            Log::error('Muzibu Category creation failed', [
                'error' => $e->getMessage(),
                'data' => $data,
                'user_id' => auth()->id()
            ]);
            ...
            return [
                'success' => false,
                'message' => __('muzibu::admin.category_create_failed'),
                'error' => $e->getMessage()
            ];
        }
    }
    ...
    public function updateCategory(int $id, array $data): bool
    {
        ...
        if ($result) {
            Log::info('Muzibu Category updated', [
                'category_id' => $id,
                'user_id' => auth()->id()
            ]);
        }
    ...
    public function deleteCategory(int $id): array
    {
        try {
            ...
            // Check if category has muzibus
            if ($category->muzibus()->count() > 0) {
                Log::warning('Cannot delete category with muzibus', [
                    'category_id' => $id,
                    'muzibu_count' => $category->muzibus()->count()
                ]);

                return [
                    'success' => false,
                    'message' => __('muzibu::admin.category_has_muzibus')
                ];
            }
            ...
            if ($result) {
                Log::info('Muzibu Category deleted', [
                    'category_id' => $id,
                    'user_id' => auth()->id()
                ]);
                ...
                return [
                    'success' => true,
                    'message' => __('muzibu::admin.category_deleted')
                ];
            }

            return [
                'success' => false,
                'message' => __('muzibu::admin.category_delete_failed')
            ];
        } catch (\Exception $e) {
            Log::error('Muzibu Category deletion failed', [
                'category_id' => $id,
                'error' => $e->getMessage(),
                'user_id' => auth()->id()
            ]);
    ...
    public function toggleCategoryStatus(int $id): array
    {
        try {
            ...
            if ($result) {
                Log::info('Muzibu Category status toggled', [
                    'category_id' => $id,
                ...
                return [
                    'success' => true,
                    'type' => 'success',
                    'message' => __('muzibu::admin.status_updated'),
                    'data' => $category,
            ...
            return [
                'success' => false,
                'type' => 'error',
                'message' => __('muzibu::admin.status_update_failed')
            ];
        } catch (\Exception $e) {
