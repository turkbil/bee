...
readonly class BlogRepository implements BlogRepositoryInterface
{
    ...
    public function findById(int $id): ?Blog
    {
        ...
        if (!$strategy->shouldCache()) {
            return $this->model->with('tags')->where('blog_id', $id)->first();
        }
        ...
        return $this->cache->remember(
            ...
            fn() => $this->model->with('tags')->where('blog_id', $id)->first()
        );
    }
    ...
    public function findByIdWithSeo(int $id): ?Blog
    {
        ...
        if ($strategy === CacheStrategy::ADMIN_FRESH) {
            return $this->model->with(['seoSetting', 'tags'])->where('blog_id', $id)->first();
        }
        ...
        return $this->cache->remember(
            ...
            $strategy->getCacheTtl(),
            fn() => $this->model->with(['seoSetting', 'tags'])->where('blog_id', $id)->first()
        );
    }
    ...
    public function getActive(): Collection
    {
        ...
        if (!$strategy->shouldCache()) {
            return $this->model->with(['tags'])->active()->orderBy('blog_id', 'desc')->get();
        }
        ...
        return Cache::tags($this->getCacheTags())
            ->remember(
                ...
                $strategy->getCacheTtl(),
                fn() =>
                $this->model->with(['tags'])->active()->orderBy('blog_id', 'desc')->get()
            );
    }
    ...
    public function getPaginated(array $filters = [], int $perPage = 10): LengthAwarePaginator
    {
        ...
        return $query->with(['seoSetting', 'tags'])->paginate($perPage);
    }
    ...
    public function update(int $id, array $data): bool
    {
        ...
        if ($result) {
            if ($tagNames !== null) {
                $blog = $this->model->with('tags')->find($id);
                if ($blog) {
                    $blog->syncTagsByName($tagNames, 'blog');
                }
            }
