...
/**
 * 🚀 Muzibu Translation Queue Job
 *
 * Bu job Muzibu modülündeki sayfaları AI ile çevirir:
 * - Toplu çeviri işlemleri için optimize edilmiş
 * - Progress tracking ile durum takibi
 * - Token kullanımı hesaplama
 */
class TranslateMuzibuJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $totalOperations = count($this->muzibuIds) * count($targetLanguages);
            $processedCount = 0;
            $successCount = 0;
            $failedCount = 0;
            $totalTokensUsed = 0;

            foreach ($this->muzibuIds as $muzibuId) {
                $muzibu = Muzibu::find($muzibuId);

                if (!$muzibu) {
                    Log::warning("Muzibu not found: {$muzibuId}");
                    $failedCount++;
                    continue;
                }
                ...
                foreach ($this->targetLanguages as $targetLanguage) {
                    ...
                    try {
                        // Kaynak içeriği al
                        $title = is_array($muzibu->title) ? ($muzibu->title[$this->sourceLanguage] ?? '') : ($muzibu->title ?? '');
                        $body = is_array($muzibu->body) ? ($muzibu->body[$this->sourceLanguage] ?? '') : ($muzibu->body ?? '');
                        ...
                        if (empty(trim($sourceData['title'] . $sourceData['body']))) {
                            Log::info("Empty content for muzibu {$muzibuId}, language {$this->sourceLanguage}");
                            $processedCount++;
                            continue;
                        }
                        ...
                        if ($response['success'] ?? false) {
                            ...
                            // Sayfayı güncelle
                            $this->updatePageTranslation($muzibu, $translatedData, $targetLanguage);
                            ...
                            try {
                                ...
                                ai_use_credits($perLanguageCost, $tenantId, [
                                    ...
                                    'description' => "Muzibu Translation: #{$muzibuId} ({$this->sourceLanguage} → {$targetLanguage})",
                                    'entity_type' => 'muzibu',
                                    'entity_id' => $muzibuId,
                                    'source_language' => $this->sourceLanguage,
                                    'target_language' => $targetLanguage,
                                    'provider_name' => 'muzibu_translation_service',
                                    'tokens_used' => $response['tokens_used'] ?? 0
                                ]);

                                Log::info('💰 KREDİ DÜŞÜRÜLDİ: MUZIBU ÇEVİRİ - 1 DİL = 1 KREDİ', [
                                    'muzibu_id' => $muzibuId,
                            ...
                            Log::info("Muzibu {$muzibuId} translated to {$targetLanguage} successfully");
                        } else {
                            $failedCount++;
                            Log::error("Translation failed for muzibu {$muzibuId} to {$targetLanguage}: " . ($response['error'] ?? 'Unknown error'));
                        }
                    } catch (Throwable $e) {
                        $failedCount++;
                        Log::error("Translation error for muzibu {$muzibuId} to {$targetLanguage}: " . $e->getMessage() . "\nStack trace: " . $e->getTraceAsString());
                    }
            ...
            event(new \Modules\Muzibu\App\Events\TranslationCompletedEvent([
     ...
     */
    private function updatePageTranslation(Muzibu $muzibu, array $translatedData, string $targetLanguage): void
    {
        // Mevcut çevirileri al
        $currentTitle = $muzibu->title ?? [];
        $currentBody = $muzibu->body ?? [];
        $currentExcerpt = $muzibu->excerpt ?? [];
        ...
        if (is_array($muzibu->slug)) {
            $slug = $muzibu->slug;
        } elseif (is_string($muzibu->slug)) {
            $slug = json_decode($muzibu->slug, true) ?? [];
        }
    ...
    public function failed(Throwable $exception): void
    {
        ...
        Log::error("TranslateMuzibuJob failed: " . $exception->getMessage());
    }
}
