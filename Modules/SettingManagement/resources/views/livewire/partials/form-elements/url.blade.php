...
@if($settingId)
    <div class="mb-3">
        <label class="form-label">{{ $element['properties']['label'] ?? 'URL' }}</label>
        <div class="input-icon">
            ...
            <input 
                ...
                class="form-control" 
                placeholder="{{ $element['properties']['placeholder'] ?? 'Web adresini girin' }}"
            >
        </div>
...
@else
    <div class="alert alert-danger">
        ...
        Bu URL alanı için ayar bulunamadı: {{ $element['properties']['name'] ?? 'Bilinmeyen' }}
    </div>
@endif