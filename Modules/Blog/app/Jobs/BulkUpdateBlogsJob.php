...
/**
 * ✏️ Bulk Blog Update Queue Job
 *
 * Blog modülünün bulk güncelleme işlemleri için queue job:
 ...
 */
class BulkUpdateBlogsJob implements ShouldQueue
{
    ...
    /**
     * @param array $blogIds Güncellenecek sayfa ID'leri
    ...
    public function handle(BlogService $blogService): void
    {
        ...
        try {
            Log::info('✏️ BULK BLOG UPDATE STARTED', [
            ...
            $progressKey = "bulk_update_blogs_{$this->tenantId}_{$this->userId}";
            ...
            foreach ($this->blogIds as $index => $blogId) {
                try {
                    ...
                    if ($this->options['validate'] ?? true) {
                        $this->validateUpdateData($blog, $filteredUpdateData);
                    }

                    // Güncelleme işlemi
                    $blog->update($filteredUpdateData);
                    log_activity($blog, 'toplu-güncellendi');
                    ...
                    $errorCount++;
                    $errors[] = "Sayfa güncelleme hatası (ID: {$blogId}): " . $e->getMessage();
            ...
            Log::info('✅ BULK BLOG UPDATE COMPLETED', [
            ...
            Log::error('💥 BULK BLOG UPDATE FAILED', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
     ...
     */
    private function validateUpdateData(Blog $blog, array $updateData): void
    {
        // Slug benzersizlik kontrolü
        if (isset($updateData['slug'])) {
            $existingPage = Blog::where('slug', $updateData['slug'])
                ->where('id', '!=', $blog->id)
                ->first();
    ...
    private function clearPageCaches(): void
    {
        try {
            ...
            Log::info('🗑️ Blog caches cleared after bulk update');
        } catch (\Exception $e) {
    ...
    {
        Log::error('💥 BULK BLOG UPDATE JOB FAILED', [
            'blog_ids' => $this->blogIds,
            'update_data' => $this->updateData,
            'tenant_id' => $this->tenantId,
            'user_id' => $this->userId,
