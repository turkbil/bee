...
class MuzibuSeeder extends Seeder
{
    public function run(): void
    {
        // Muzibu SADECE tenant database'lerde olmalı
        if (\App\Helpers\TenantHelpers::isCentral()) {
            $this->command->info('📦 Muzibu: sadece tenant database için, atlanıyor...');
            return;
        }
        ...
        if (!tenancy()->initialized) {
            $this->command->error('❌ Tenant context not initialized for Muzibu!');
            return;
        }

        // Duplicate check - eğer zaten veri varsa skip
        if (MuzibuCategory::count() > 0) {
            $this->command->warn("⚠️  Muzibu categories already exist. Skipping...");
            return;
        }
        ...
        foreach ($categoryNames as $nameTr => $nameEn) {
            $category = MuzibuCategory::create([
                'title' => [
                    'tr' => $nameTr,
                    'en' => $nameEn,
                    'ar' => $nameTr
