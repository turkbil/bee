...
namespace Modules\Muzibu\App\Http\Livewire\Traits;
 ...
 *
 * Muzibu modülü için queue-based bulk işlemler:
 * - Bulk delete (queue)
 * - Bulk update (queue)
 * - Progress tracking
...
trait WithBulkActionsQueue
{
    ...
    public function bulkDelete()
    {
        ...
        try {
            ...
            \Modules\Muzibu\App\Jobs\BulkDeleteMuzibusJob::dispatch(
                $this->selectedItems,
                $tenantId,
                $userId,
                ['force_delete' => false]
            ...
            $this->bulkProgress = [
                'operation' => 'delete',
                'count' => $count,
                'progress_key' => "bulk_delete_muzibus_{$tenantId}_{$userId}",
    ...
    public function bulkUpdate($updateData)
    {
        ...
        try {
            ...
            \Modules\Muzibu\App\Jobs\BulkUpdateMuzibusJob::dispatch(
                $this->selectedItems,
                $updateData,
                $tenantId,
                $userId,
            ...
            $this->bulkProgress = [
                'operation' => 'update',
                'count' => $count,
                'progress_key' => "bulk_update_muzibus_{$tenantId}_{$userId}",
                'started_at' => now()->toISOString()
            ];
     ...
     */
    public function translateContent($data, ?int $muzibuId = null): void
    {
        if (!$muzibuId) {
            $this->dispatch('toast', [
                'title' => 'Çeviri Hatası',
        ...
        try {
            ...
            $progressKey = "muzibu_translation_progress_{$muzibuId}_" . uniqid();

            // Translation job dispatch et
            $job = \Modules\Muzibu\App\Jobs\TranslateMuzibuContentJob::dispatch($data, $muzibuId);
