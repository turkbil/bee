...
return new class extends Migration
{
    ...
    {
        Schema::create('muzibu_categories', function (Blueprint $table) {
            ...
            // Foreign key
            $table->foreign('parent_id')->references('category_id')->on('muzibu_categories')->onDelete('cascade');

            // İlave indeksler
            $table->index('created_at');
            $table->index('updated_at');
            $table->index('deleted_at');
            $table->index(['is_active', 'deleted_at', 'sort_order'], 'muzibu_categories_active_deleted_sort_idx');
        });
        ...
        if (DB::getDriverName() === 'mysql') {
            ...
            if ($supportsJsonIndex) {
                ...
                foreach ($systemLanguages as $locale) {
                    DB::statement("
                        ALTER TABLE muzibu_categories
                        ADD INDEX muzibu_categories_slug_{$locale} (
    ...
    {
        Schema::dropIfExists('muzibu_categories');
    }
};
