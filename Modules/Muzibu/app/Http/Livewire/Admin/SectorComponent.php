...
use Modules\Muzibu\App\Services\SectorService;
...
class SectorComponent extends Component
{
    ...
    public function boot(SectorService $sectorService): void
    {
    ...
    public function refreshPageData()
    {
        ...
        $this->sectorService->clearCache();
        $this->render();
    }

    public function handleTranslationCompleted($eventData)
    {
        Log::info('🎉 MUZIBU: SectorComponent - TranslationCompleted event received', $eventData);

        $this->dispatch('translation-complete', [
            'success' => true,
            'sessionId' => $eventData['sessionId'] ?? null,
            'entityType' => $eventData['entityType'] ?? 'sector',
    ...
    {
        return \Modules\Muzibu\App\Models\Sector::class;
    }
    ...
    {
        try {
            $result = $this->sectorService->toggleSectorStatus($id);
    ...
    public function render(): \Illuminate\Contracts\View\View
    {
        ...
        $sectors = $this->sectorService->getPaginatedSectors($filters, (int) $this->perPage);

        return view('muzibu::admin.livewire.sector-component', [
            'sectors' => $sectors,
