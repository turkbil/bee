<?php
    ...
    class EFL352_Forklift_2_Detailed extends Seeder {
        public function run(): void {
            $p = DB::table('shop_products')->where('sku', 'EFL352')->first();
            if (!$p) {$this->command->error('❌ Master bulunamadı: EFL352'); return; }

            DB::table('shop_products')->where('product_id', $p->product_id)->update([
                'long_description' => json_encode(['tr' => '<section>
    <h2>İXTİF EFL352: Dizel Alışkanlığını Li-Ion Gücüyle Değiştirin</h2>
    <p>Depo kapıları sabah açıldığında, hız ve dayanıklılık kadar süreklilik de kritik hale gelir. İXTİF EFL352, içten yanmalı forkliftlerden alışık olduğunuz sağlam sürüş hissini, LFP Li-Ion batarya mimarisi ve IPX4 su korumalı gövde ile birleştirir. Büyük lastikler ve yüksek yerden yükseklik, açık sahada çukurları tolere ederken, dar alanlarda 2645 mm dönüş yarıçapı ile atik manevralar sağlar. Fırsat şarj kabiliyeti sayesinde operasyon aralarında dakikalarla ölçülen kısa şarjlar tüm vardiyanızı besler; böylece plan dışı duruşlar tarihe karışır.</p>
</section>
<section>
    <h3>Teknik Güç ve Verimlilik</h3>
    <p>3500 kg nominal kapasite ve AC sürüş kontrolü ile operatör, yük altında bile akıcı hızlanma ve kontrollü yavaşlama elde eder. 11/12 km/sa seyir hızı günlük taşıma akışlarını hızlandırırken, 0.26/0.34 m/s (yük/yüksüz) kaldırma değerleri istif döngüsünü kısaltır. LFP kimya yapısı, termal kaçak riskini düşürürken ömür boyu bakım gerektiren filtre ve yağ değişimlerini ortadan kaldırır. IPX4 koruma sınıfı, yağmur altında dahi güven veren bir yapı sunar; mekanik park freni ve hidrolik servis freni ise operatöre güvenli, öngörülebilir duruş mesafeleri sağlar. Basit komponent mimarisi ve iyi erişilebilen servis noktaları bakım süresini kısaltır, toplam sahip olma maliyetini düşürür.</p>
</section>
<section>
    <h3>Sonuç: Bugün Planlayın, Yarın Kazanın</h3>
    <p>Telematik paketi, araç konumu, kullanım ve akü sağlığına ilişkin verileri görünür kılarak filonuzu ölçülebilir kılar. Böylece bakım randevularını önceden planlar, kullanım dışı süreleri minimumda tutarsınız. Projelerinize uygun direk ve çatal alternatifleriyle EFL352, saha gerçeklerine uyum sağlayan modüler bir çözümdür. Bilgi ve teklif için 0216 755 3 555.</p>
</section>'], JSON_UNESCAPED_UNICODE),
                'primary_specs' => json_encode([
                    ['icon' => 'weight-hanging', 'label' => 'Kapasite', 'value' => '3500 kg'],
                    ['icon' => 'battery-full', 'label' => 'Batarya', 'value' => '80V 280Ah Li-Ion (LFP)'],
                    ['icon' => 'gauge', 'label' => 'Hız', 'value' => '11/12 km/sa (yük/yüksüz)'],
                    ['icon' => 'arrows-turn-right', 'label' => 'Dönüş', 'value' => '2645 mm']
            ...
            $this->command->info("✅ Detailed güncellendi: EFL352");
        }
    }
