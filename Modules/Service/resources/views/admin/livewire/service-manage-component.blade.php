<div>
    @php
        View::share(
            'pretitle',
            $serviceId ? __('service::admin.edit_service_pretitle') : __('service::admin.new_service_pretitle'),
        );
    @endphp
    ...
    <form method="post" wire:submit.prevent="save">
        ...
        <div class="card">
            ...
            <x-tab-system :tabs="$tabConfig" :tab-completion="$tabCompletionStatus" storage-key="service_active_tab">
                {{-- Studio Edit Button --}}
                @if ($studioEnabled && $serviceId)
                    <li class="nav-item ms-3">
                        <a href="{{ route('admin.studio.editor', ['module' => 'service', 'id' => $serviceId]) }}"
                            target="_blank" class="btn btn-outline-primary"
                            style="padding: 0.20rem 0.75rem; margin-top: 5px;">
                            <i
                                class="fa-solid fa-wand-magic-sparkles fa-lg me-1"></i>{{ __('service::admin.studio.editor') }}
                        </a>
                    </li>
                @endif
            ...
            <div class="card-body">
                <div class="tab-content" id="contentTabContent">
                    ...
                    <div class="tab-pane show active" id="0" role="tabpanel">
                        @foreach ($availableLanguages as $lang)
                            ...
                            <div class="language-content" data-language="{{ $lang }}"
                                ...
                                <div class="row mb-4">
                                    <div class="col-12 col-md-6">
                                        <div class="form-floating">
                                            <select wire:model="inputs.service_category_id"
                                                class="form-control @error('inputs.service_category_id') is-invalid @enderror"
                                                id="category_select">
                                                <option value="">{{ __('service::admin.select_category') }}</option>
                                            ...
                                            <label for="category_select">
                                                {{ __('service::admin.category') }}
                                            </label>
                                            @error('inputs.service_category_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    </div>
                        ...
                        <div class="mb-4">
                            <livewire:mediamanagement::universal-media
                                wire:id="service-media-component"
                                :model-id="$serviceId"
                                model-type="service"
                                model-class="Modules\Service\App\Models\Service"
                                :collections="['hero', 'gallery']"
                                :sortable="true"
                                :set-featured-from-gallery="true"
                                :key="'universal-media-' . ($serviceId ?? 'new')"
                            />
                        </div>
                        ...
                        @foreach ($availableLanguages as $lang)
                            ...
                            <div class="language-content" data-language="{{ $lang }}"
                                ...
                                @include('admin.components.content-editor', [
                                    ...
                                    'langData' => $langData,
                                    'fieldName' => 'body',
                                    'label' => __('service::admin.content'),
                                    'placeholder' => __('service::admin.content_placeholder'),
                                ])
                            </div>
                        @endforeach
                        ...
                        <div class="mb-3 mt-4">
                            <div class="pretty p-default p-curve p-toggle p-smooth ms-1">
                                ...
                                <div class="state p-success p-on ms-2">
                                    <label>{{ __('service::admin.active') }}</label>
                                </div>
                                <div class="state p-danger p-off ms-2">
                                    <label>{{ __('service::admin.inactive') }}</label>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane" id="1" role="tabpanel">
                        <livewire:seomanagement::universal-seo-tab :model-id="$serviceId" model-type="service"
                            model-class="Modules\Service\App\Models\Service" />
                    </div>
            ...
            <x-form-footer route="admin.service" :model-id="$serviceId" />
    ...
    @push('scripts')
        {{-- 🎯 MODEL & MODULE SETUP --}}
        <script>
            window.currentModelId = {{ $serviceId ?? 'null' }};
            window.currentModuleName = 'service';
