@php
    ...
    try {
        $istifCategory = \Modules\Shop\app\Models\ShopCategory::where('slug', 'istif-makinesi')
            ->where('is_active', 1)
            ->first();
...
<div class="w-full">
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            @if($istifCategory && $istifCategory->description)
                <p class="text-gray-600 dark:text-gray-400 mt-2">{{ Str::limit($istifCategory->description, 120) }}</p>
            @endif
        </div>
        <a href="{{ route('shop.index') }}?category=istif-makinesi" class="bg-purple-600 dark:bg-purple-700 text-white px-6 py-3 rounded-xl font-bold hover:bg-purple-700 dark:hover:bg-purple-800 transition flex items-center gap-2">
            ...
            @foreach($subCategories as $index => $subCat)
                ...
                <a href="{{ route('shop.category', $subCat->slug) }}" class="group">
                    <div class="bg-gradient-to-br from-{{ $color['bg'] }}-50 to-{{ $color['bg'] }}-100 dark:from-{{ $color['bg'] }}-900/30 dark:to-{{ $color['bg'] }}-900/20 rounded-2xl p-8 group-hover:shadow-xl transition-all duration-300 h-48 flex items-center justify-center mb-4 relative overflow-hidden">
                        ...
                        @else
                            <i class="fa-solid {{ $subCat->icon ?? 'fa-box-open-full' }} text-7xl text-{{ $color['icon'] }}-400 dark:text-{{ $color['icon'] }}-500 group-hover:text-{{ $color['icon'] }}-600 dark:group-hover:text-{{ $color['icon'] }}-400 group-hover:scale-110 transition-all duration-300"></i>
                        @endif
                    </div>
    ...
    @else
        {{-- Fallback content --}}
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            <div class="text-center p-8 bg-purple-50 dark:bg-purple-900/20 rounded-2xl border-2 border-dashed border-purple-200 dark:border-purple-700">
                <i class="fa-solid fa-box-open-full text-6xl text-purple-500 dark:text-purple-400 mb-4"></i>
                <h5 class="font-bold text-gray-800 dark:text-gray-200 mb-2">İstif Makinesi Kategorileri</h5>
