 ...
 *
 * ReviewSystem modülü için queue-based bulk işlemler:
 * - Bulk delete (queue)
 * - Bulk update (queue)
 * - Progress tracking
...
trait WithBulkActionsQueue
{
    ...
    public function bulkDelete()
    {
        ...
        try {
            ...
            \Modules\ReviewSystem\App\Jobs\BulkDeleteReviewSystemsJob::dispatch(
                $this->selectedItems,
                $tenantId,
                $userId,
                ['force_delete' => false]
            ...
            $this->bulkProgress = [
                'operation' => 'delete',
                'count' => $count,
                'progress_key' => "bulk_delete_reviewsystems_{$tenantId}_{$userId}",
    ...
    public function bulkUpdate($updateData)
    {
        ...
        try {
            ...
            \Modules\ReviewSystem\App\Jobs\BulkUpdateReviewSystemsJob::dispatch(
                $this->selectedItems,
                $updateData,
                $tenantId,
                $userId,
            ...
            $this->bulkProgress = [
                'operation' => 'update',
                'count' => $count,
                'progress_key' => "bulk_update_reviewsystems_{$tenantId}_{$userId}",
                'started_at' => now()->toISOString()
            ];
     ...
     */
    public function translateContent($data, ?int $reviewsystemId = null): void
    {
        if (!$reviewsystemId) {
            $this->dispatch('toast', [
                'title' => 'Çeviri Hatası',
        ...
        try {
            ...
            $progressKey = "reviewsystem_translation_progress_{$reviewsystemId}_" . uniqid();

            // Translation job dispatch et
            $job = \Modules\ReviewSystem\App\Jobs\TranslateReviewSystemContentJob::dispatch($data, $reviewsystemId);
