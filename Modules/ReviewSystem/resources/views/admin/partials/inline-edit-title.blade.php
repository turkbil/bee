<td wire:key="title-{{ $reviewsystem->reviewsystem_id }}" class="position-relative">
        ...
        <div class="d-flex align-items-center gap-3" x-data @click.outside="$wire.updateTitleInline()">
            <div class="flexible-input-wrapper">
                <input type="text"
                       ...
                       placeholder="{{ __('reviewsystem::admin.enter_new_title') }}"
    ...
    @else
        <div class="d-flex align-items-center">
            <span class="editable-title pr-4">{{ $reviewsystem->getTranslated('title') }}</span>
            <button class="btn btn-sm px-2 py-1 edit-icon ms-4"
                wire:click="startEditingTitle({{ $reviewsystem->reviewsystem_id }}, '{{ addslashes($reviewsystem->getTranslated('title')) }}')">
