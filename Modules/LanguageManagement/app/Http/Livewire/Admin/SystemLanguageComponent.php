...
#[Layout('admin.layout')]
class SystemLanguageComponent extends Component
{
    use WithPagination;
    ...
    public function delete($id)
    {
        ...
        if ($systemLanguageService->deleteSystemLanguage($language->code)) {
    ...
    public function toggleActive($id)
    {
        ...
        if ($language->save()) {
            $systemLanguageService = app(SystemLanguageService::class);
            $systemLanguageService->clearSystemLanguageCache();
    ...
    public function updateOrder($itemIds)
    {
        ...
        if (function_exists('log_activity') && !empty($itemIds)) {
            $firstLanguage = SystemLanguage::find($itemIds[0]);
            if ($firstLanguage) {
