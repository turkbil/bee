...
/**
 * Announcement Bulk Operations Tests
 *
 * Toplu işlemlerin (bulk operations) doğru çalışmasını
 */
class AnnouncementBulkOperationsTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function it_can_bulk_toggle_page_status(): void
    {
        ...
        $ids = $activePages->pluck('announcement_id')->toArray();
        ...
        foreach ($activePages as $announcement) {
    ...
    public function it_can_toggle_both_active_and_inactive_pages(): void
    {
        ...
        $ids = $mixedPages->pluck('announcement_id')->toArray();
    ...
    {
        $announcement = Announcement::factory()->create();
        $announcement->getOrCreateSeoSetting(); // SEO ayarı ekle

        $this->assertDatabaseHas('seo_settings', [
            'seo_settingable_id' => $announcement->announcement_id
        ]);

        $this->service->bulkDeletePages([$announcement->announcement_id]);
    ...
    public function bulk_operations_return_correct_result_objects(): void
    {
        ...
        $this->assertInstanceOf(\Modules\Announcement\App\DataTransferObjects\BulkOperationResult::class, $result);
        $this->assertTrue($result->success);
