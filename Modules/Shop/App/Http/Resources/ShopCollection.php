...
/**
 * Shop Collection Resource
 *
 * Transforms collection of shops for API responses.
...
class ShopCollection extends ResourceCollection
{
    ...
    private function getMeta(Request $request): array
    {
        ...
        if ($this->resource instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            $meta['pagination'] = [
                ...
                'per_shop' => $this->resource->perPage(),
                'current_shop' => $this->resource->currentPage(),
                'total_shops' => $this->resource->lastPage(),
            ];
        } else {
    ...
    {
        // Add cache headers for list shops
        if (!config('app.debug')) {
            $ttl = config('shop.cache.ttl.list', 3600);
