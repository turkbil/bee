    ...
    <article class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <header class="mb-8">
            ...
            <div class="mt-4 flex flex-wrap gap-4 text-sm text-gray-600 dark:text-gray-400">
                ...
                <span>{{ __('shop::front.general.category') }}: {{ optional($item->category)->getTranslated('title', app()->getLocale()) ?? '—' }}</span>
                <span>{{ __('shop::front.general.brand') }}: {{ optional($item->brand)->getTranslated('title', app()->getLocale()) ?? '—' }}</span>
            </div>
        </header>

        <section class="prose prose-lg dark:prose-invert max-w-none mb-12">
            {!! $item->getTranslated('long_description', app()->getLocale()) !!}
        </section>

        {{-- Features --}}
        @if($item->features && isset($item->features[app()->getLocale()]))
            <section class="mb-12">
                <h2 class="text-2xl font-bold text-gray-900 dark:text-white mb-6">{{ __('shop::front.general.features') }}</h2>
            ...
            <section class="mb-12">
                ...
                <div class="space-y-4">
                    @foreach($item->faq_data as $faq)
                        <details class="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-5">
            ...
            <section class="mb-12">
                ...
                <div class="flex flex-wrap gap-2">
                    @foreach($item->target_industries[app()->getLocale()] as $industry)
                        <span class="inline-flex items-center px-4 py-2 bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300 rounded-full text-sm font-medium border border-gray-200 dark:border-gray-700">
                            {{ $industry }}
                        </span>
                    @endforeach
                </div>
        ...
        <footer class="mt-10 border-t border-gray-200 dark:border-gray-700 pt-6 text-sm text-gray-500 dark:text-gray-400">
            <div class="flex flex-wrap gap-4">
                <span>{{ __('shop::front.general.updated_at') }}: {{ $item->updated_at?->translatedFormat('d F Y') }}</span>
                <span>{{ __('shop::front.general.published_at') }}: {{ $item->published_at?->translatedFormat('d F Y') }}</span>
            </div>
        </footer>
    </article>
@endsection
