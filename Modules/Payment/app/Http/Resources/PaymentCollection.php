 ...
 *
 * Transforms collection of payments for API responses.
...
class PaymentCollection extends ResourceCollection
{
    ...
    private function getMeta(Request $request): array
    {
        ...
        if ($this->resource instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            $meta['pagination'] = [
                ...
                'per_payment' => $this->resource->perPage(),
                'current_payment' => $this->resource->currentPage(),
                'total_payments' => $this->resource->lastPage(),
            ];
        } else {
    ...
    {
        // Add cache headers for list payments
        if (!config('app.debug')) {
            $ttl = config('payment.cache.ttl.list', 3600);
