...
use Modules\Muzibu\App\Services\ArtistService;
...
class ArtistComponent extends Component
{
    ...
    public function boot(ArtistService $artistService): void
    {
    ...
    public function refreshPageData()
    {
        ...
        $this->artistService->clearCache();
        $this->render();
    }

    public function handleTranslationCompleted($eventData)
    {
        Log::info('🎉 MUZIBU: ArtistComponent - TranslationCompleted event received', $eventData);

        $this->dispatch('translation-complete', [
            'success' => true,
            'sessionId' => $eventData['sessionId'] ?? null,
            'entityType' => $eventData['entityType'] ?? 'artist',
    ...
    {
        return \Modules\Muzibu\App\Models\Artist::class;
    }
    ...
    {
        try {
            $result = $this->artistService->toggleArtistStatus($id);
    ...
    public function render(): \Illuminate\Contracts\View\View
    {
        ...
        $artists = $this->artistService->getPaginatedArtists($filters, (int) $this->perPage);

        return view('muzibu::admin.livewire.artist-component', [
            'artists' => $artists,
