 ...
 *
 * User tipine g�re dynamic M3U8 playlist olu_turur
 * Guest/Normal User: 4 chunk (3 �al + 1 buffer)
 * Premium User: T�m chunk'lar
 */
class PlaylistGeneratorService
{
    ...
    public function generatePlaylist($song, $user = null): string
    {
        ...
        // User tipine g�re chunk limiti belirle
        $chunkLimit = $this->getChunkLimit($user);

        // Playlist'i limit'e g�re filter et
    ...
    protected function getChunkLimit($user): ?int
    {
        ...
        if (!$user || !$user->isPremium()) {
            // 4 chunk = 3 �alacak (30 saniye) + 1 buffer (10 saniye)
            return 4;
        }
    ...
    /**
     * Playlist'i chunk limitine g�re filtrele
     *
    ...
    {
        // Limit yoksa olduu gibi d�nd�r
        if ($limit === null) {
            return $playlist;
        }
    ...
    protected function signChunkUrls(string $playlist, int $songId, $user): string
    {
        ...
        // segment-000.ts veya chunk_000.ts vb. t�m chunk dosyalar1n1 bul
    ...
    /**
     * Chunk index'i chunk dosya ad1ndan �1kar
     * segment-005.ts � 5
     *
