...
@section('title', 'Uyelikleri Yonet - ' . ($account->company_name ?? 'Kurumsal'))

@section('content')
{{-- JSON Data Element (SPA uyumlu) --}}
<script type="application/json" id="corporate-subscriptions-data">
{
    "plans": @json($plans),
...
document.addEventListener('alpine:init', () => {
    Alpine.data('corporateSubscriptions', () => ({
        // Data (JSON'dan yuklenecek)
        plans: [],
        members: [],
        ...
        selectAll() {
            this.selectedUserIds = this.members.map(m => m.user_id);
        },
        selectNone() {
        ...
        purchase() {
            console.log('🔵 Purchase clicked', {
                ...
                cycleKey: this.selectedCycleKey,
                userIds: this.selectedUserIds
            });
            if (!this.canPurchase || this.purchasing) {
                console.warn('⚠️ Purchase blocked:', {canPurchase: this.canPurchase, purchasing: this.purchasing});
                return;
            }
            this.purchasing = true;

            // 🔥 WORKAROUND: Sepete GET parametreleriyle redirect et
            // Backend controller'da zaten subscription parametresi varsa sepete ekliyor
            ...
            const checkoutUrl = '/cart/checkout?' + params.toString();
...
<div x-data="corporateSubscriptions" x-init="init()" class="min-h-screen pb-24">
        ...
        <div class="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8 pb-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <h1 class="text-3xl font-bold text-white">Uyelikleri Yonet</h1>
                    <p class="text-gray-400 mt-1">{{ $account->company_name ?? 'Kurumsal' }} - Ekibiniz icin premium uyelik satin alin</p>
                </div>
                <div class="flex items-center gap-3 px-4 py-2 bg-purple-500/10 border border-purple-500/30 rounded-xl">
                    ...
                    <span class="text-purple-300 font-medium">{{ count($members) }} Uye</span>
                </div>
            </div>
        </div>
    </div>
        ...
        @if(empty($members))
            <div class="max-w-md mx-auto text-center py-16">
                <div class="bg-slate-800/50 rounded-2xl p-8 border border-white/10">
                    ...
                    <h2 class="text-2xl font-bold text-white mb-2">Henuz Uye Yok</h2>
                    <p class="text-gray-400 mb-6">Kurumsal hesabiniza henuz uye eklenmemis.</p>
                        ...
                        <div class="grid sm:grid-cols-2 gap-4">
                            @foreach($plans as $index => $plan)
                                ...
                                <div @click="selectPlanById({{ $planId }})"
                                     ...
                                     class="relative rounded-xl p-5 cursor-pointer transition-all border group">
                                    ...
                                    <div class="flex items-start justify-between mb-3">
                                        ...
                                        <div x-show="selectedPlanId == {{ $planId }}"
                                             class="w-6 h-6 bg-gradient-to-r from-purple-500 to-pink-500 rounded-full flex items-center justify-center flex-shrink-0">
                                            <i class="fas fa-check text-white text-xs"></i>
                                        </div>
                                    </div>

                                    {{-- Fiyat (sadece secili degilken) --}}
                                    @if($firstCycle)
                                        <div x-show="selectedPlanId != {{ $planId }}" class="flex items-baseline gap-1">
                                    ...
                                    {{-- Donem Secimi (sadece seciliyken) --}}
                                    <div x-show="selectedPlanId == {{ $planId }}" x-collapse class="mt-2">
                                        <div class="flex flex-wrap gap-2">
                                            @foreach($cycles as $cycleKey => $cycle)
                                                <button type="button"
                                                    ...
                                                    {{ $cycle['label']['tr'] ?? $cycleKey }} - {{ number_format($cycle['price'] ?? 0, 0, ',', '.') }} TL
                                                </button>
                                            @endforeach
                                        </div>
                                    </div>
                ...
                <div class="bg-slate-800/50 rounded-2xl border border-white/10 overflow-hidden">
                    <div class="px-5 py-4 border-b border-white/10 bg-white/5 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <h2 class="text-lg font-semibold text-white flex items-center">
                            ...
                            Uyeleri Secin
                            <span class="ml-2 text-sm font-normal text-gray-400">({{ count($members) }} uye)</span>
                        </h2>
                        <div class="flex gap-2">
                            <button @click="selectExpired()" class="px-3 py-1.5 bg-red-500/10 hover:bg-red-500/20 text-red-400 rounded-lg text-xs font-medium transition border border-red-500/20">
                                <i class="fas fa-clock mr-1"></i>Suresi Dolanlar
                            </button>
                        ...
                        <div class="space-y-3 max-h-[500px] overflow-y-auto scrollbar-hide">
                            @foreach($members as $index => $member)
                                ...
                                <div @click="toggleUser({{ $member['user_id'] }})"
                                     ...
                                     class="flex items-center gap-4 p-4 rounded-xl cursor-pointer transition-all border">

                                    {{-- Checkbox --}}
                                    <div :class="isUserSelected({{ $member['user_id'] }})
                                    ...
                                    @if($member['subscription']['is_active'])
                                        @if($member['subscription']['status'] === 'expiring')
                ...
                <div class="bg-slate-800/50 rounded-2xl sticky top-6 overflow-hidden border border-white/10">
                    ...
                    <div class="p-5 border-b border-white/10">
                        <div class="space-y-4">
                            ...
                            {{-- Donem --}}
                            <div class="flex justify-between items-center">
                                <span class="text-gray-400 text-sm">Odeme Donemi</span>
                                <span class="text-white font-medium" x-text="selectedCycle?.label?.tr || '-'"></span>
                            </div>
                            {{-- Uye Basina --}}
                            <div class="flex justify-between items-center">
                                <span class="text-gray-400 text-sm">Uye Basina</span>
                    ...
                    <div class="p-5 border-b border-white/10">
                        <div class="flex justify-between items-center mb-3">
                            <span class="text-gray-400 text-sm">Secilen Uyeler</span>
                            <span class="text-2xl font-bold text-white" x-text="selectedCount"></span>
                        </div>

                        {{-- Secilen Uye Listesi (max 5) --}}
                        <div x-show="selectedCount > 0" class="space-y-2">
                            <template x-for="(userId, idx) in selectedUserIds.slice(0, 5)" :key="userId">
                                <div class="flex items-center gap-2 text-sm">
                                    ...
                                    <span class="text-gray-300 truncate" x-text="members.find(m => m.user_id === userId)?.name || 'Uye'"></span>
                                </div>
                            </template>
                    ...
                    <div class="p-5 bg-white/5">
                        ...
                        <button @click="purchase()"
                            ...
                            <span x-show="!purchasing"><i class="fas fa-lock mr-2"></i>Odemeye Gec</span>
                            <span x-show="purchasing"><i class="fas fa-spinner fa-spin mr-2"></i>Yukleniyor...</span>
                        </button>
