 ...
 *
 * @property \Modules\Payment\App\Models\Payment $resource
 */
class PaymentResource extends JsonResource
{
    ...
    private function getAttributes(string $locale, array $fields): array
    {
        $attributes = [
            ...
            'body' => $this->when(
                !isset($fields['payments']) || in_array('body', $fields['payments'] ?? []),
                fn() => $this->getTranslatedAttribute('body', $locale)
            ),
            'css' => $this->when(
                !isset($fields['payments']) || in_array('css', $fields['payments'] ?? []),
                $this->css
            ),
            'js' => $this->when(
                !isset($fields['payments']) || in_array('js', $fields['payments'] ?? []),
                $this->js
            ),
    ...
    private function getRelationships(array $includeRelations): array
    {
        ...
        if ($this->relationLoaded('seoSetting') || in_array('seo', $includeRelations)) {
            $relationships['seo'] = [
                'links' => [
                    'self' => route('api.payments.relationships.seo', $this->payment_id),
                    'related' => route('api.payments.seo', $this->payment_id)
                ],
                'data' => $this->when(
        ...
        if (in_array('activities', $includeRelations)) {
            $relationships['activities'] = [
                'links' => [
                    'self' => route('api.payments.relationships.activities', $this->payment_id),
                    'related' => route('api.payments.activities', $this->payment_id)
                ],
                'meta' => [
    ...
    private function getLinks(string $locale): array
    {
        ...
        return [
            'self' => route('api.payments.show', $this->payment_id),
            'frontend' => $slug ? route('payment.show', $slug) : null,
            'admin' => route('admin.payment.manage', $this->payment_id)
        ];
    }
    ...
    /**
     * Get available locales for this payment
     *
     * @return array<string>
     */
