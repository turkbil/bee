 ...
 *
 * Transforms collection of services for API responses.
...
class ServiceCollection extends ResourceCollection
{
    ...
    private function getMeta(Request $request): array
    {
        ...
        if ($this->resource instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            $meta['pagination'] = [
                ...
                'per_service' => $this->resource->perPage(),
                'current_service' => $this->resource->currentPage(),
                'total_services' => $this->resource->lastPage(),
            ];
        } else {
    ...
    {
        // Add cache headers for list services
        if (!config('app.debug')) {
            $ttl = config('service.cache.ttl.list', 3600);
