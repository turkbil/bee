...
class ES12_12WA_Istif_2_Detailed extends Seeder {
    public function run(): void {
        $p = DB::table('shop_products')->where('sku', 'ES12-12WA')->first();
        if (!$p) {$this->command->error('❌ Master bulunamadı: ES12-12WA'); return; }

        DB::table('shop_products')->where('product_id', $p->product_id)->update([
            'long_description' => json_encode(['tr' => '<section><h2>Operasyonların Ritmini Yükselten Akıllı İstif</h2>
<p>Günün ilk sevkiyatı kapıya yanaşırken, ekip raf yenilemeye çoktan başlar. İXTİF ES12-12WA dar koridorlarda sakin ama
emin adımlarla ilerler; AC sürüş sistemi yük altındayken bile akıcı hızlanma sağlar. 600&nbsp;mm yük merkezi ve
1200 kg nominal kapasite dengeli istifi mümkün kılar. İki kademeli indirme, üst raflara yaklaşırken
...
2.2 kW (S3 15%) gücündeki hidrolik ünite üstlenir; bu sayede yüklü/boş kaldırma hızları 0.10 / 0.16 m/s (yüklü/boş)
...
1970 mm ve açıkken 3420 mm değerlerine ulaşır. Gövde yerden yüksekliği (m2)
28 mm düzeyinde olduğundan rampalarda paletin sürtünmesini azaltır. Ana şasi ölçüleri (l1)
1826 mm ve (l2) 676 mm, koridorda konumlamayı kolaylaştırır. Elektromanyetik park
...
<p>İş gücü verimliliği artık saniyelerle ölçülüyor. İXTİF ES12-12WA, güvenlik ve hız arasında ideal dengeyi kurarak toplam
...
JSON
        ...
        $this->command->info('✅ Detailed güncellendi: ES12-12WA');
    }
}
