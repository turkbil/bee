...
/**
 * ✏️ Bulk Muzibu Update Queue Job
 *
 * Muzibu modülünün bulk güncelleme işlemleri için queue job:
 ...
 */
class BulkUpdateMuzibusJob implements ShouldQueue
{
    ...
    /**
     * @param array $muzibuIds Güncellenecek sayfa ID'leri
    ...
    public function handle(MuzibuService $muzibuService): void
    {
        ...
        try {
            Log::info('✏️ BULK MUZIBU UPDATE STARTED', [
            ...
            $progressKey = "bulk_update_muzibus_{$this->tenantId}_{$this->userId}";
            ...
            foreach ($this->muzibuIds as $index => $muzibuId) {
                try {
                    ...
                    if ($this->options['validate'] ?? true) {
                        $this->validateUpdateData($muzibu, $filteredUpdateData);
                    }

                    // Güncelleme işlemi
                    $muzibu->update($filteredUpdateData);
                    log_activity($muzibu, 'toplu-güncellendi');
                    ...
                    $errorCount++;
                    $errors[] = "Sayfa güncelleme hatası (ID: {$muzibuId}): " . $e->getMessage();
            ...
            Log::info('✅ BULK MUZIBU UPDATE COMPLETED', [
            ...
            Log::error('💥 BULK MUZIBU UPDATE FAILED', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
     ...
     */
    private function validateUpdateData(Muzibu $muzibu, array $updateData): void
    {
        // Slug benzersizlik kontrolü
        if (isset($updateData['slug'])) {
            $existingPage = Muzibu::where('slug', $updateData['slug'])
                ->where('id', '!=', $muzibu->id)
                ->first();
    ...
    private function clearPageCaches(): void
    {
        try {
            ...
            Log::info('🗑️ Muzibu caches cleared after bulk update');
        } catch (\Exception $e) {
    ...
    {
        Log::error('💥 BULK MUZIBU UPDATE JOB FAILED', [
            'muzibu_ids' => $this->muzibuIds,
            'update_data' => $this->updateData,
            'tenant_id' => $this->tenantId,
            'user_id' => $this->userId,
