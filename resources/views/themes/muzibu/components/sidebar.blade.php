<aside class="fixed left-0 z-40 h-full w-64 bg-black {{ auth()->check() ? 'top-0' : 'top-14' }}">
    <div class="flex flex-col h-full">
        ...
        <nav class="flex-1 overflow-y-auto py-4 px-3">
            <ul class="space-y-1">
                <li><a href="/" :class="currentPath === '/' ? 'text-white font-semibold bg-muzibu-gray' : 'text-gray-400 hover:text-white'" class="flex items-center px-4 py-2 transition-all rounded-md"><i class="fas fa-home w-5 mr-3"></i>{{ tenant_trans('front.home') }}</a></li>
                <li><a href="/search" :class="currentPath === '/search' ? 'text-white font-semibold bg-muzibu-gray' : 'text-gray-400 hover:text-white'" class="flex items-center px-4 py-2 transition-all rounded-md"><i class="fas fa-search w-5 mr-3"></i>{{ tenant_trans('front.search') }}</a></li>
                <li><a href="/playlists" :class="currentPath === '/playlists' ? 'text-white font-semibold bg-muzibu-gray' : 'text-gray-400 hover:text-white'" class="flex items-center px-4 py-2 transition-all rounded-md"><i class="fas fa-list-music w-5 mr-3"></i>{{ tenant_trans('front.playlists') }}</a></li>
                <li><a href="/albums" :class="currentPath === '/albums' ? 'text-white font-semibold bg-muzibu-gray' : 'text-gray-400 hover:text-white'" class="flex items-center px-4 py-2 transition-all rounded-md"><i class="fas fa-record-vinyl w-5 mr-3"></i>{{ tenant_trans('front.albums') }}</a></li>
                <li><a href="/genres" :class="currentPath === '/genres' ? 'text-white font-semibold bg-muzibu-gray' : 'text-gray-400 hover:text-white'" class="flex items-center px-4 py-2 transition-all rounded-md"><i class="fas fa-guitar w-5 mr-3"></i>{{ tenant_trans('front.genres') }}</a></li>
                <li><a href="/sectors" :class="currentPath === '/sectors' ? 'text-white font-semibold bg-muzibu-gray' : 'text-gray-400 hover:text-white'" class="flex items-center px-4 py-2 transition-all rounded-md"><i class="fas fa-building w-5 mr-3"></i>{{ tenant_trans('front.sectors') }}</a></li>
            </ul>
            ...
            <div class="mt-6 px-3 space-y-3">
                <a href="/register" class="block w-full px-4 py-3 rounded-full bg-gradient-to-r from-muzibu-coral to-green-600 hover:from-muzibu-coral-light hover:to-green-500 text-white font-bold transition-all shadow-lg text-center">
                    <i class="fas fa-rocket mr-2"></i>{{ tenant_trans('front.free_trial') }}
                </a>
                <a href="/login" class="block w-full px-4 py-3 rounded-full bg-transparent border border-gray-600 text-white font-semibold hover:border-white hover:scale-105 transition-all text-center">
            ...
            @auth
            <div class="mt-6 px-3">
                <div class="p-4 bg-muzibu-gray rounded-lg">
                    <!-- Normal State -->
                    ...
                    <template x-if="isLoggingOut">
                        <div class="flex items-center justify-center py-4">
                            <i class="fas fa-spinner fa-spin text-muzibu-coral text-xl mr-2"></i>
