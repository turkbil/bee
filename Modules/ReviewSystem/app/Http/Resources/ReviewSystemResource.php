...
/**
 * ReviewSystem Resource
 *
 * Transforms ReviewSystem model for API responses.
 * Implements JSON API specification.
 *
 * @property \Modules\ReviewSystem\App\Models\ReviewSystem $resource
 */
class ReviewSystemResource extends JsonResource
{
    /**
    ...
    private function getAttributes(string $locale, array $fields): array
    {
        $attributes = [
            ...
            'body' => $this->when(
                !isset($fields['reviewsystems']) || in_array('body', $fields['reviewsystems'] ?? []),
                fn() => $this->getTranslatedAttribute('body', $locale)
            ),
            'css' => $this->when(
                !isset($fields['reviewsystems']) || in_array('css', $fields['reviewsystems'] ?? []),
                $this->css
            ),
            'js' => $this->when(
                !isset($fields['reviewsystems']) || in_array('js', $fields['reviewsystems'] ?? []),
                $this->js
            ),
    ...
    private function getRelationships(array $includeRelations): array
    {
        ...
        if ($this->relationLoaded('seoSetting') || in_array('seo', $includeRelations)) {
            $relationships['seo'] = [
                'links' => [
                    'self' => route('api.reviewsystems.relationships.seo', $this->reviewsystem_id),
                    'related' => route('api.reviewsystems.seo', $this->reviewsystem_id)
                ],
                'data' => $this->when(
        ...
        if (in_array('activities', $includeRelations)) {
            $relationships['activities'] = [
                'links' => [
                    'self' => route('api.reviewsystems.relationships.activities', $this->reviewsystem_id),
                    'related' => route('api.reviewsystems.activities', $this->reviewsystem_id)
                ],
                'meta' => [
    ...
    private function getLinks(string $locale): array
    {
        ...
        return [
            'self' => route('api.reviewsystems.show', $this->reviewsystem_id),
            'frontend' => $slug ? route('reviewsystem.show', $slug) : null,
            'admin' => route('admin.reviewsystem.manage', $this->reviewsystem_id)
        ];
    }
    ...
    /**
     * Get available locales for this reviewsystem
     *
     * @return array<string>
     */
