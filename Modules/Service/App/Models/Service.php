...
class Service extends BaseModel implements TranslatableEntity, HasMedia
{
    ...
    /**
     * ID accessor - service_id'yi id olarak döndür
     */
    ...
    {
        return $this->belongsTo(ServiceCategory::class, 'service_category_id', 'category_id');
    }
    ...
    {
        // Check if service has any images in content
        $content = $this->getTranslated('body', app()->getLocale()) ?? $this->body;
    ...
    public function getAllSchemas(): array
    {
        ...
        $serviceSchema = $this->getSchemaMarkup();
        if ($serviceSchema) {
            $schemas['creativework'] = $serviceSchema;
        }
    ...
    /**
     * Get or create SEO setting for this service
     */
    ...
    {
        // Service modülü için özel işlemler burada yapılabilir
        // Örneğin: Cache temizleme, sitemap güncelleme vb.

        \Log::info("Service çevirisi tamamlandı", [
    ...
    /**
     * Service için locale-aware URL oluştur
    ...
    public function getUrl(?string $locale = null): string
    {
        ...
        $moduleSlug = \App\Services\ModuleSlugService::getSlug('Service', 'show');
