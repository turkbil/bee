<div>
    @php
        View::share(
            'pretitle',
            $favoriteId ? __('favorite::admin.edit_favorite_pretitle') : __('favorite::admin.new_favorite_pretitle'),
        );
    @endphp
    ...
    <form method="post" wire:submit.prevent="save">
        ...
        <div class="card">
            ...
            <x-tab-system :tabs="$tabConfig" :tab-completion="$tabCompletionStatus" storage-key="favorite_active_tab">
                {{-- Studio Edit Button --}}
                @if ($studioEnabled && $favoriteId)
                    <li class="nav-item ms-3">
                        <a href="{{ route('admin.studio.editor', ['module' => 'favorite', 'id' => $favoriteId]) }}"
                            target="_blank" class="btn btn-outline-primary"
                            style="padding: 0.20rem 0.75rem; margin-top: 5px;">
                            <i
                                class="fa-solid fa-wand-magic-sparkles fa-lg me-1"></i>{{ __('favorite::admin.studio.editor') }}
                        </a>
                    </li>
                @endif
            ...
            <div class="card-body">
                <div class="tab-content" id="contentTabContent">
                    ...
                    <div class="tab-pane show active" id="0" role="tabpanel">
                        ...
                        <div class="mb-4">
                            <livewire:mediamanagement::universal-media
                                wire:id="favorite-media-component"
                                :model-id="$favoriteId"
                                model-type="favorite"
                        ...
                        @foreach ($availableLanguages as $lang)
                            ...
                            <div class="language-content" data-language="{{ $lang }}"
                                ...
                                @include('admin.components.content-editor', [
                                    ...
                                    'langData' => $langData,
                                    'fieldName' => 'body',
                                    'label' => __('favorite::admin.content'),
                                    'placeholder' => __('favorite::admin.content_placeholder'),
                                ])
                            </div>
                        @endforeach
                        ...
                        <div class="mb-3 mt-4">
                            <div class="pretty p-default p-curve p-toggle p-smooth ms-1">
                                ...
                                <div class="state p-success p-on ms-2">
                                    <label>{{ __('favorite::admin.active') }}</label>
                                </div>
                                <div class="state p-danger p-off ms-2">
                                    <label>{{ __('favorite::admin.inactive') }}</label>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="tab-pane" id="1" role="tabpanel">
                        <livewire:seomanagement::universal-seo-tab :model-id="$favoriteId" model-type="favorite"
                            model-class="Modules\Favorite\App\Models\Favorite" />
                    </div>
            ...
            <x-form-footer route="admin.favorite" :model-id="$favoriteId" />
    ...
    @push('scripts')
        {{-- 🎯 MODEL & MODULE SETUP --}}
        <script>
            window.currentModelId = {{ $favoriteId ?? 'null' }};
            window.currentModuleName = 'favorite';
