...
/**
 * ReviewSystem Permission Tests
 *
 * Yetkilendirme sisteminin doğru çalıştığını
 * ve erişim kontrollerini test eder.
 */
class ReviewSystemPermissionTest extends TestCase
{
    use RefreshDatabase;
    ...
    public function admin_can_create_reviewsystem(): void
    {
        ...
        $reviewsystem = ReviewSystem::factory()->make();

        $this->assertDatabaseCount('reviewsystems', 0);

        ReviewSystem::create([
            'title' => $reviewsystem->title,
            'slug' => $reviewsystem->slug,
            'body' => $reviewsystem->body,
            'is_active' => true,
        ]);

        $this->assertDatabaseCount('reviewsystems', 1);
    }
    ...
    public function admin_can_update_reviewsystem(): void
    {
        ...
        $reviewsystem->update(['title' => ['tr' => 'Updated Title', 'en' => 'Updated Title']]);

        $this->assertEquals('Updated Title', $reviewsystem->fresh()->getTranslated('title', 'tr'));
    }

    /** @test */
    public function admin_can_delete_reviewsystem(): void
    {
        $this->actingAs($this->admin);
    ...
    /** @test */
    public function viewer_can_view_reviewsystems_but_cannot_edit(): void
    {
    ...
    public function activity_logging_records_admin_actions(): void
    {
        ...
        if (function_exists('log_activity')) {
            log_activity($reviewsystem, 'created');

            // Activity log database'de olmalı
            $this->assertDatabaseHas('activity_log', [
                'subject_type' => get_class($reviewsystem),
                'subject_id' => $reviewsystem->reviewsystem_id
            ]);
        } else {
    ...
    public function xss_protection_is_active(): void
    {
        ...
        $this->assertStringNotContainsString('<script>', $reviewsystem->fresh()->getTranslated('body', 'tr'));
    }
    ...
    public function sql_injection_protection_works(): void
    {
        ...
        $maliciousInput = "'; DROP TABLE reviewsystems; --";
    ...
    public function mass_assignment_protection_works(): void
    {
        ...
        $reviewsystem = ReviewSystem::create([
            ...
            'body' => ['tr' => 'Test', 'en' => 'Test'],
            'is_active' => true,
            // reviewsystem_id manuel set edilemez (primary key)
            'reviewsystem_id' => 9999,
        ]);

        // reviewsystem_id otomatik generate edilmeli, manuel değer ignore edilmeli
        $this->assertNotEquals(9999, $reviewsystem->reviewsystem_id);
    }
    ...
    {
        $routes = [
            route('admin.reviewsystem.index'),
            route('admin.reviewsystem.manage'),
        ];
