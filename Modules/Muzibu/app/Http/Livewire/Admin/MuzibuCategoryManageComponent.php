...
#[Layout('admin.layout')]
class MuzibuCategoryManageComponent extends Component
{
    public $categoryId;
    ...
    public function currentCategory()
    {
        ...
        return MuzibuCategory::query()->find($this->categoryId);
    }
    ...
    {
        $categories = MuzibuCategory::orderBy('sort_order', 'asc')
    ...
    {
        $this->categoryService = app(\Modules\Muzibu\App\Services\MuzibuCategoryService::class);

        view()->share('pretitle', __('muzibu::admin.category_management'));
        view()->share('title', __('muzibu::admin.categories'));
    }
    ...
    protected function initializeUniversalComponents()
    {
        ...
        $this->tabConfig = \App\Services\GlobalTabService::getAllTabs('muzibu_category');
        $this->activeTab = \App\Services\GlobalTabService::getDefaultTabKey('muzibu_category');
    }
    ...
    public function handleLanguageChange($language)
    {
        if (in_array($language, $this->availableLanguages)) {
            ...
            Log::info('MuzibuCategoryManage - Dil değişti', [
    ...
    {
        $category = MuzibuCategory::find($id);
    ...
    protected function rules()
    {
        $rules = [
            ...
            'inputs.parent_id' => 'nullable|exists:muzibu_categories,category_id',
        ];
    ...
    public function save($redirect = false, $resetForm = false)
    {
        ...
        if ($this->categoryId) {
            $category = MuzibuCategory::query()->findOrFail($this->categoryId);
            $category->update($data);
            log_activity($category, 'güncellendi');

            $toast = [
                'title' => __('admin.success'),
                'message' => __('muzibu::admin.category_updated'),
                'type' => 'success'
            ];
        } else {
            $category = MuzibuCategory::query()->create($data);
            $this->categoryId = $category->category_id;
            log_activity($category, 'eklendi');

            $toast = [
                'title' => __('admin.success'),
                'message' => __('muzibu::admin.category_created'),
                'type' => 'success'
            ];
        }
        ...
        if ($redirect) {
            ...
            return redirect()->route('admin.muzibu.category.index');
        }
    ...
    {
        return view('muzibu::admin.livewire.category-manage-component', [
            'jsVariables' => [
    ...
    public function deleteCategory()
    {
        if (!$this->categoryId) {
            $this->dispatch('toast', [
                ...
                'message' => __('muzibu::admin.category_not_found'),
                'type' => 'error'
            ]);
            return;
        }
