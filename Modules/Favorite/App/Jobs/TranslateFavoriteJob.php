...
/**
 * 🚀 Favorite Translation Queue Job
 *
 * Bu job Favorite modülündeki sayfaları AI ile çevirir:
 * - Toplu çeviri işlemleri için optimize edilmiş
 * - Progress tracking ile durum takibi
 * - Token kullanımı hesaplama
 */
class TranslateFavoriteJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $totalOperations = count($this->favoriteIds) * count($targetLanguages);
            $processedCount = 0;
            $successCount = 0;
            $failedCount = 0;
            $totalTokensUsed = 0;

            foreach ($this->favoriteIds as $favoriteId) {
                $favorite = Favorite::find($favoriteId);

                if (!$favorite) {
                    Log::warning("Favorite not found: {$favoriteId}");
                    $failedCount++;
                    continue;
                }
                ...
                foreach ($this->targetLanguages as $targetLanguage) {
                    ...
                    try {
                        // Kaynak içeriği al
                        $title = is_array($favorite->title) ? ($favorite->title[$this->sourceLanguage] ?? '') : ($favorite->title ?? '');
                        $body = is_array($favorite->body) ? ($favorite->body[$this->sourceLanguage] ?? '') : ($favorite->body ?? '');
                        ...
                        if (empty(trim($sourceData['title'] . $sourceData['body']))) {
                            Log::info("Empty content for favorite {$favoriteId}, language {$this->sourceLanguage}");
                            $processedCount++;
                            continue;
                        }
                        ...
                        if ($response['success'] ?? false) {
                            ...
                            // Sayfayı güncelle
                            $this->updatePageTranslation($favorite, $translatedData, $targetLanguage);
                            ...
                            try {
                                ...
                                ai_use_credits($perLanguageCost, $tenantId, [
                                    ...
                                    'description' => "Favorite Translation: #{$favoriteId} ({$this->sourceLanguage} → {$targetLanguage})",
                                    'entity_type' => 'favorite',
                                    'entity_id' => $favoriteId,
                                    'source_language' => $this->sourceLanguage,
                                    'target_language' => $targetLanguage,
                                    'provider_name' => 'favorite_translation_service',
                                    'tokens_used' => $response['tokens_used'] ?? 0
                                ]);

                                Log::info('💰 KREDİ DÜŞÜRÜLDİ: FAVORITE ÇEVİRİ - 1 DİL = 1 KREDİ', [
                                    'favorite_id' => $favoriteId,
                            ...
                            Log::info("Favorite {$favoriteId} translated to {$targetLanguage} successfully");
                        } else {
                            $failedCount++;
                            Log::error("Translation failed for favorite {$favoriteId} to {$targetLanguage}: " . ($response['error'] ?? 'Unknown error'));
                        }
                    } catch (Throwable $e) {
                        $failedCount++;
                        Log::error("Translation error for favorite {$favoriteId} to {$targetLanguage}: " . $e->getMessage() . "\nStack trace: " . $e->getTraceAsString());
                    }
            ...
            event(new \Modules\Favorite\App\Events\TranslationCompletedEvent([
     ...
     */
    private function updatePageTranslation(Favorite $favorite, array $translatedData, string $targetLanguage): void
    {
        // Mevcut çevirileri al
        $currentTitle = $favorite->title ?? [];
        $currentBody = $favorite->body ?? [];
        $currentExcerpt = $favorite->excerpt ?? [];
        ...
        if (is_array($favorite->slug)) {
            $slug = $favorite->slug;
        } elseif (is_string($favorite->slug)) {
            $slug = json_decode($favorite->slug, true) ?? [];
        }
        ...
        log_activity($favorite, 'çevrildi');
    }
    ...
    public function failed(Throwable $exception): void
    {
        ...
        Log::error("TranslateFavoriteJob failed: " . $exception->getMessage());
    }
}
