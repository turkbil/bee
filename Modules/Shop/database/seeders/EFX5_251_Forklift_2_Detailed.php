...
class EFX5_251_Forklift_2_Detailed extends Seeder {
    public function run(): void {
        $p = DB::table('shop_products')->where('sku', 'EFX5-251')->first();
        if (!$p) {$this->command->error('❌ Master bulunamadı: EFX5-251'); return; }

        DB::table('shop_products')->where('product_id', $p->product_id)->update([
            'long_description' => json_encode(['tr' => '<section>
  <h2>İXTİF EFX5 251: Modüler Li-Ion ile kesintisiz verim</h2>
  <p>Depo kapıları açıldığında iş temposu başlar ve durmaz. İXTİF EFX5 251, modüler 80V Li-Ion akü mimarisi, güçlü
...
<section>
  ...
  16/17 km/s seyir, 2500 kg kg taşıma ve AC tahrik kontrolü; hızlanma ve hassas manevrada tutarlı sonuç verir.
  2270 mm dönüş yarıçapı dar koridorlarda etkili konumlanma sağlar. Triplex mastın artırılmış görüş alanı ve
...
<section>
  <h3>Sonuç</h3>
  <p>İş hacminiz değişse bile EFX5 251 esner: tek bataryayla ekonomik kullanım, çift bataryayla uzun dayanım.
...
JSON
        ...
        $this->command->info("✅ Detailed güncellendi: EFX5-251");
    }
}
