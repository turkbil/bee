<td wire:key="title-{{ $shop->shop_id }}" class="position-relative">
    @if ($editingTitleId === $shop->shop_id)
        <div class="d-flex align-items-center gap-3" x-data @click.outside="$wire.updateTitleInline()">
            <div class="flexible-input-wrapper">
                <input type="text"
                       ...
                       placeholder="{{ __('shop::admin.enter_new_title') }}"
    ...
    @else
        <div class="d-flex align-items-center">
            <span class="editable-title pr-4">{{ $shop->getTranslated('title') }}</span>
            <button class="btn btn-sm px-2 py-1 edit-icon ms-4"
                wire:click="startEditingTitle({{ $shop->shop_id }}, '{{ addslashes($shop->getTranslated('title')) }}')">
