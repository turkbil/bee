<div>
    ...
    @include('muzibu::admin.helper-album')
    ...
    <form method="post" wire:submit.prevent="save">
        ...
        <div class="card">
            ...
            <x-tab-system :tabs="$tabConfig" :tab-completion="$tabCompletionStatus" storage-key="album_active_tab">
                {{-- Studio Edit Button --}}
                @if ($studioEnabled && $albumId)
                    <li class="nav-item ms-3">
                        <a href="{{ route('admin.studio.editor', ['module' => 'album', 'id' => $albumId]) }}"
                            target="_blank" class="btn btn-outline-primary"
                            style="padding: 0.20rem 0.75rem; margin-top: 5px;">
                            <i
                                class="fa-solid fa-wand-magic-sparkles fa-lg me-1"></i>{{ __('muzibu::admin.album.studio.editor') }}
                        </a>
                    </li>
                @endif
                        ...
                        @foreach ($availableLanguages as $lang)
                            ...
                            <div class="language-content" data-language="{{ $lang }}"
                                ...
                                <div class="row mb-4">
                                    <div class="col-12 col-md-6">
                                        <div class="form-floating">
                                            <select wire:model="inputs.album_category_id"
                                                class="form-control @error('inputs.album_category_id') is-invalid @enderror"
                                                id="category_select">
                                                <option value="">{{ __('muzibu::admin.album.select_category') }}</option>
                                            ...
                                            <label for="category_select">
                                                {{ __('muzibu::admin.album.category') }}
                                            </label>
                                            @error('inputs.album_category_id')
                                                <div class="invalid-feedback">{{ $message }}</div>
                                            @enderror
                                        </div>
                                    </div>
