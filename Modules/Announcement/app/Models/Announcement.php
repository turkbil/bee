...
class Announcement extends BaseModel implements TranslatableEntity, HasMedia
{
    ...
    public function getSeoFallbackSchemaMarkup(): ?array
    {
        ...
        if ($this->created_at) {
            $schema['datePublished'] = $this->created_at->toIso8601String();
        }
    ...
    /**
     * Tüm schema'ları al (Article + Breadcrumb)
     */
    public function getAllSchemas(): array
    {
        $schemas = [];

        // 1. Article Schema (Ana içerik)
        $announcementSchema = $this->getSchemaMarkup();
        if ($announcementSchema) {
            $schemas['article'] = $announcementSchema;
        }
