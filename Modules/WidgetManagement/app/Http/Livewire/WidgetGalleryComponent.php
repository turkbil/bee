...
class WidgetGalleryComponent extends Component
{
    ...
    public function render()
    {
        ...
        // Gallery tipindeki widgetları getir (sayılar için)
        $galleryWidgetsQuery = Widget::where('type', '!=', 'file')
            ->where('type', '!=', 'module')
        ...
        $parentCategories->each(function($category) use ($galleryWidgetsQuery) {
            ...
            $childWidgetsCount = Widget::whereIn('widget_category_id', $childCategoryIds)
                ->whereIn('id', $galleryWidgetsQuery->pluck('id'))
                ->count();
