...
#[Layout('admin.layout')]
class ShopCategoryManageComponent extends Component
{
    public $categoryId;
    ...
    public function currentCategory()
    {
        ...
        return ShopCategory::query()->find($this->categoryId);
    }
    ...
    {
        $categories = ShopCategory::orderBy('sort_order', 'asc')
    ...
    {
        $this->categoryService = app(\Modules\Shop\App\Services\ShopCategoryService::class);

        view()->share('pretitle', __('shop::admin.category_management'));
        view()->share('title', __('shop::admin.categories'));
    }
    ...
    protected function initializeUniversalComponents()
    {
        ...
        $this->tabConfig = \App\Services\GlobalTabService::getAllTabs('shop_category');
        $this->activeTab = \App\Services\GlobalTabService::getDefaultTabKey('shop_category');
    }
    ...
    public function handleLanguageChange($language)
    {
        if (in_array($language, $this->availableLanguages)) {
            ...
            Log::info('ShopCategoryManage - Dil değişti', [
    ...
    protected function rules()
    {
        $rules = [
            ...
            'inputs.parent_id' => 'nullable|exists:shop_categories,category_id',
        ];
    ...
    public function save($redirect = false, $resetForm = false)
    {
        ...
        if ($this->categoryId) {
            $category = ShopCategory::query()->findOrFail($this->categoryId);
            $category->update($data);
            log_activity($category, 'güncellendi');

            $toast = [
                'title' => __('admin.success'),
                'message' => __('shop::admin.category_updated'),
                'type' => 'success'
            ];
        } else {
            $category = ShopCategory::query()->create($data);
    ...
    {
        return view('shop::admin.livewire.category-manage-component', [
            'jsVariables' => [
