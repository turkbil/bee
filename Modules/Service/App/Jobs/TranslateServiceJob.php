...
/**
 * 🚀 Service Translation Queue Job
 *
 * Bu job Service modülündeki sayfaları AI ile çevirir:
 * - Toplu çeviri işlemleri için optimize edilmiş
 * - Progress tracking ile durum takibi
 * - Token kullanımı hesaplama
 */
class TranslateServiceJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $totalOperations = count($this->serviceIds) * count($targetLanguages);
            $processedCount = 0;
            $successCount = 0;
            $failedCount = 0;
            $totalTokensUsed = 0;
            ...
            foreach ($this->serviceIds as $serviceId) {
                ...
                foreach ($this->targetLanguages as $targetLanguage) {
                    ...
                    try {
                        // Kaynak içeriği al
                        $title = is_array($service->title) ? ($service->title[$this->sourceLanguage] ?? '') : ($service->title ?? '');
                        $body = is_array($service->body) ? ($service->body[$this->sourceLanguage] ?? '') : ($service->body ?? '');
                        ...
                        if (empty(trim($sourceData['title'] . $sourceData['body']))) {
                            Log::info("Empty content for service {$serviceId}, language {$this->sourceLanguage}");
                            $processedCount++;
                            continue;
                        }
                        ...
                        if ($response['success'] ?? false) {
                            ...
                            // Sayfayı güncelle
                            $this->updatePageTranslation($service, $translatedData, $targetLanguage);
                            ...
                            try {
                                ...
                                ai_use_credits($perLanguageCost, $tenantId, [
                                    ...
                                    'description' => "Service Translation: #{$serviceId} ({$this->sourceLanguage} → {$targetLanguage})",
                                    'entity_type' => 'service',
                                    'entity_id' => $serviceId,
                                    'source_language' => $this->sourceLanguage,
                                    'target_language' => $targetLanguage,
                                    'provider_name' => 'service_translation_service',
                                    'tokens_used' => $response['tokens_used'] ?? 0
                                ]);

                                Log::info('💰 KREDİ DÜŞÜRÜLDİ: SERVICE ÇEVİRİ - 1 DİL = 1 KREDİ', [
                                    'service_id' => $serviceId,
                            ...
                            Log::info("Service {$serviceId} translated to {$targetLanguage} successfully");
                        } else {
                            $failedCount++;
                            Log::error("Translation failed for service {$serviceId} to {$targetLanguage}: " . ($response['error'] ?? 'Unknown error'));
                        }
                    } catch (Throwable $e) {
                        $failedCount++;
                        Log::error("Translation error for service {$serviceId} to {$targetLanguage}: " . $e->getMessage() . "\nStack trace: " . $e->getTraceAsString());
                    }
            ...
            event(new \Modules\Service\App\Events\TranslationCompletedEvent([
     ...
     */
    private function updatePageTranslation(Service $service, array $translatedData, string $targetLanguage): void
    {
        // Mevcut çevirileri al
        $currentTitle = $service->title ?? [];
        $currentBody = $service->body ?? [];
        $currentExcerpt = $service->excerpt ?? [];
        ...
        if (is_array($service->slug)) {
            $slug = $service->slug;
        } elseif (is_string($service->slug)) {
            $slug = json_decode($service->slug, true) ?? [];
        }
    ...
    public function failed(Throwable $exception): void
    {
        ...
        Log::error("TranslateServiceJob failed: " . $exception->getMessage());
    }
}
