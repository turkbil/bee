...
const StudioBlocks = (function() {
    ...
    function registerBlocks(editor) {
        ...
        // Blok panelini manuel olarak yapılandıralım
        const blockManager = editor.BlockManager;
        
        // Tüm kaydedilen blokları al
        ...
        // Her kategori için blokları grupla
        const categories = editor.BlockManager.getCategories().models;
        ...
        categories.forEach(category => {
            ...
            // Kategoriye ait blokları filtrele
            ...
            if (categoryBlocks.length > 0) {
                ...
                blockHTML += `
                    <div class="block-category mb-3">
                        <h6 class="block-category-title mb-2">${categoryLabel}</h6>
                        <div class="block-items row g-2">
                `;
                ...
                categoryBlocks.forEach(block => {
                    ...
                    blockHTML += `
                        <div class="col-6">
                            <div class="block-item card" data-block-id="${blockId}">
                                <div class="card-body p-2 text-center">
                                    <i class="fas fa-cube mb-2"></i>
                                    <div class="block-label small">${blockLabel}</div>
                                </div>
                            </div>
                        </div>
                    `;
        ...
        // Blok konteynerine HTML'i ekle
        const blocksContainer = document.getElementById('blocks-container');
        if (blocksContainer) {
            blocksContainer.innerHTML = blockHTML;
            
            // Blok öğelerine sürükle bırak işlevselliği ekle
            document.querySelectorAll('.block-item').forEach(blockItem => {
                blockItem.addEventListener('mousedown', function(e) {
                    ...
                    if (block) {
                        editor.runCommand('drag-items', { target: block });
                    }
                });
            });
        } else {
            console.error('Blok konteyneri bulunamadı: blocks-container');
        }
