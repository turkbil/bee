...
/**
 * Muzibu Category Seeder
 *
 * Temel muzibu kategorilerini oluşturur.
 * Tüm kategoriler JSON çoklu dil desteği ile gelir.
 *
 * @package Modules\Muzibu\Database\Seeders
 */
class MuzibuCategorySeeder extends Seeder
{
    private $fakers = [];
    private $languages = [];
    ...
    {
        // Muzibu kategorileri SADECE tenant database'lerde olmalı
        if (\App\Helpers\TenantHelpers::isCentral()) {
            $this->command->info('📁 Muzibu categories: sadece tenant database için, atlanıyor...');
            return;
        }

        // Central tenant (ID=1 / laravel database) kontrolü
        if (tenancy()->initialized && tenant('tenancy_db_name') === 'laravel') {
            $this->command->error('❌ Central tenant detected, muzibu tables do not exist in central!');
            return;
        }

        // Tenant context kontrolü
        if (!tenancy()->initialized) {
            $this->command->error('❌ Tenant context not initialized for Muzibu Categories!');
            return;
        }

        // Duplicate check
        if (MuzibuCategory::count() > 3) {
            $this->command->warn("⚠️  Muzibu categories exist (>3). Skipping...");
            return;
        }
        ...
        $this->command->info("📁 Creating muzibu categories for languages: " . implode(', ', $this->languages));
        ...
        foreach ($categories as $categoryData) {
            ...
            $category = MuzibuCategory::create($categoryData);
    ...
    private function createSeoSettings($category, $seoMeta): void
    {
        foreach ($seoMeta as $lang => $data) {
            $category->seoSetting()->updateOrCreate(
                ['seoable_id' => $category->category_id, 'seoable_type' => MuzibuCategory::class],
                [
