...
/**
 * Portfolio Bulk Operations Tests
 *
 * Toplu işlemlerin (bulk operations) doğru çalışmasını
 */
class PortfolioBulkOperationsTest extends TestCase
{
    use RefreshDatabase;
    ...
    /** @test */
    public function it_can_bulk_delete_multiple_portfolios(): void
    {
    ...
    /** @test */
    public function it_can_bulk_toggle_portfolio_status(): void
    {
        $activePages = Portfolio::factory()->active()->count(3)->create();
        $inactivePages = Portfolio::factory()->inactive()->count(2)->create();

        $ids = $activePages->pluck('portfolio_id')->toArray();
        ...
        foreach ($activePages as $portfolio) {
            $this->assertFalse($portfolio->fresh()->is_active);
        }
    }

    /** @test */
    public function it_can_toggle_both_active_and_inactive_portfolios(): void
    {
        $mixedPages = collect([
        ...
        $ids = $mixedPages->pluck('portfolio_id')->toArray();
    ...
    {
        $portfolio = Portfolio::factory()->create();
        $portfolio->getOrCreateSeoSetting(); // SEO ayarı ekle

        $this->assertDatabaseHas('seo_settings', [
            'seo_settingable_id' => $portfolio->portfolio_id
        ]);

        $this->service->bulkDeletePages([$portfolio->portfolio_id]);
    ...
    public function bulk_operations_return_correct_result_objects(): void
    {
        ...
        $this->assertInstanceOf(\Modules\Portfolio\App\DataTransferObjects\BulkOperationResult::class, $result);
        $this->assertTrue($result->success);
    ...
    public function partial_bulk_delete_returns_partial_result(): void
    {
        ...
        $allIds = array_merge([$homeportfolio->portfolio_id], $portfolios->pluck('portfolio_id')->toArray());
    ...
    {
        $portfolios = Portfolio::factory()->count(100)->create();
        $ids = $portfolios->pluck('portfolio_id')->toArray();
    ...
    public function bulk_toggle_maintains_data_integrity(): void
    {
        ...
        foreach ($portfolios as $portfolio) {
            ...
            $this->assertEquals($portfolio->getTranslated('title', 'tr'), $fresh->getTranslated('title', 'tr'));
        }
    }
