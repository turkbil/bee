...
/**
 * 🚀 Blog Translation Queue Job
 *
 * Bu job Blog modülündeki sayfaları AI ile çevirir:
...
class TranslateBlogJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $totalOperations = count($this->blogIds) * count($targetLanguages);
            $processedCount = 0;
            $successCount = 0;
            $failedCount = 0;
            $totalTokensUsed = 0;
            ...
            foreach ($this->blogIds as $blogId) {
                ...
                if (!$blog) {
                    Log::warning("Blog not found: {$blogId}");
                    $failedCount++;
                    continue;
                }
                ...
                foreach ($this->targetLanguages as $targetLanguage) {
                    ...
                    try {
                        // Kaynak içeriği al
                        $title = is_array($blog->title) ? ($blog->title[$this->sourceLanguage] ?? '') : ($blog->title ?? '');
                        $body = is_array($blog->body) ? ($blog->body[$this->sourceLanguage] ?? '') : ($blog->body ?? '');
                        ...
                        if (empty(trim($sourceData['title'] . $sourceData['body']))) {
                            Log::info("Empty content for blog {$blogId}, language {$this->sourceLanguage}");
                            $processedCount++;
                            continue;
                        }
                        ...
                        if ($response['success'] ?? false) {
                            ...
                            // Sayfayı güncelle
                            $this->updatePageTranslation($blog, $translatedData, $targetLanguage);
                            ...
                            try {
                                ...
                                ai_use_credits($perLanguageCost, $tenantId, [
                                    ...
                                    'description' => "Blog Translation: #{$blogId} ({$this->sourceLanguage} → {$targetLanguage})",
                                    'entity_type' => 'blog',
                                    'entity_id' => $blogId,
                                    'source_language' => $this->sourceLanguage,
                                    'target_language' => $targetLanguage,
                                    'provider_name' => 'blog_translation_service',
                                    'tokens_used' => $response['tokens_used'] ?? 0
                                ]);

                                Log::info('💰 KREDİ DÜŞÜRÜLDİ: BLOG ÇEVİRİ - 1 DİL = 1 KREDİ', [
                                    'blog_id' => $blogId,
                            ...
                            Log::info("Blog {$blogId} translated to {$targetLanguage} successfully");
                        } else {
                            $failedCount++;
                            Log::error("Translation failed for blog {$blogId} to {$targetLanguage}: " . ($response['error'] ?? 'Unknown error'));
                        }
                    } catch (Throwable $e) {
                        $failedCount++;
                        Log::error("Translation error for blog {$blogId} to {$targetLanguage}: " . $e->getMessage() . "\nStack trace: " . $e->getTraceAsString());
                    }
            ...
            event(new \Modules\Blog\App\Events\TranslationCompletedEvent([
     ...
     */
    private function updatePageTranslation(Blog $blog, array $translatedData, string $targetLanguage): void
    {
        // Mevcut çevirileri al
        $currentTitle = $blog->title ?? [];
        $currentBody = $blog->body ?? [];
        $currentExcerpt = $blog->excerpt ?? [];
        ...
        if (is_array($blog->slug)) {
            $slug = $blog->slug;
        } elseif (is_string($blog->slug)) {
            $slug = json_decode($blog->slug, true) ?? [];
        }
    ...
    public function failed(Throwable $exception): void
    {
        ...
        Log::error("TranslateBlogJob failed: " . $exception->getMessage());
    }
}
