 ...
 * Portfolio Resource
 *
 * Transforms Portfolio model for API responses.
 * Implements JSON API specification.
 *
 * @property \Modules\Portfolio\App\Models\Portfolio $resource
 */
class PortfolioResource extends JsonResource
{
    ...
    private function getAttributes(string $locale, array $fields): array
    {
        $attributes = [
            ...
            'body' => $this->when(
                !isset($fields['portfolios']) || in_array('body', $fields['portfolios'] ?? []),
                fn() => $this->getTranslatedAttribute('body', $locale)
            ),
            'css' => $this->when(
                !isset($fields['portfolios']) || in_array('css', $fields['portfolios'] ?? []),
                $this->css
            ),
            'js' => $this->when(
                !isset($fields['portfolios']) || in_array('js', $fields['portfolios'] ?? []),
                $this->js
            ),
