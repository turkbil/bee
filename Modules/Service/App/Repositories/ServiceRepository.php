...
use Modules\Service\App\Enums\CacheStrategy;

readonly class ServiceRepository implements ServiceRepositoryInterface
{
    ...
    public function __construct(
        ...
        $this->cachePrefix = TenantCacheService::PREFIX_SERVICE;
    ...
    public function findById(int $id): ?Service
    {
        ...
        return $this->cache->remember(
            ...
            fn() => $this->model->where('service_id', $id)->first()
        );
    }

    public function findByIdWithSeo(int $id): ?Service
    {
        $strategy = CacheStrategy::fromRequest();

        // Admin panelinde cache kullanma
        if ($strategy === CacheStrategy::ADMIN_FRESH) {
            return $this->model->with('seoSetting')->where('service_id', $id)->first();
        }
        ...
        return $this->cache->remember(
            ...
            $strategy->getCacheTtl(),
            fn() => $this->model->with('seoSetting')->where('service_id', $id)->first()
        );
    }

    public function findBySlug(string $slug, string $locale = 'tr'): ?Service
    {
    ...
    public function getActive(): Collection
    {
        ...
        if (!$strategy->shouldCache()) {
            return $this->model->active()->orderBy('service_id', 'desc')->get();
        }

        $cacheKey = $this->getCacheKey('active_services');
        ...
        return Cache::tags($this->getCacheTags())
            ->remember(
                ...
                $strategy->getCacheTtl(),
                fn() =>
                $this->model->active()->orderBy('service_id', 'desc')->get()
            );
    }
    ...
    public function getPaginated(array $filters = [], int $perPage = 10): LengthAwarePaginator
    {
        ...
        // Sorting
        $sortField = $filters['sortField'] ?? 'service_id';
    ...
    {
        $result = $this->model->where('service_id', $id)->update($data);
    ...
    {
        $result = $this->model->where('service_id', $id)->delete();
    ...
    public function toggleActive(int $id): bool
    {
        ...
        $service = $this->model->where('service_id', $id)->first(['service_id', 'is_active']);

        if (!$service) {
            return false;
        }

        $result = $this->model->where('service_id', $id)->update(['is_active' => !$service->is_active]);
    ...
    {
        $count = $this->model->whereIn('service_id', $ids)->delete();
    ...
    public function bulkToggleActive(array $ids): int
    {
        ...
        $services = $this->model->whereIn('service_id', $ids)->get(['service_id', 'is_active']);
        $count = 0;

        foreach ($services as $service) {
            $this->model->where('service_id', $service->service_id)
    ...
    {
        $service = $this->model->where('service_id', $id)->first(['service_id', 'seo']);
        ...
        $result = $this->model->where('service_id', $id)->update(['seo' => $seo]);
