...
namespace Modules\ReviewSystem\App\Repositories;
...
use Modules\ReviewSystem\App\Enums\CacheStrategy;

readonly class ReviewSystemRepository implements ReviewSystemRepositoryInterface
{
    ...
    public function __construct(
        private ReviewSystem $model
    ) {
        $this->cachePrefix = TenantCacheService::PREFIX_REVIEWSYSTEM;
    ...
    public function findById(int $id): ?ReviewSystem
    {
        ...
        if (!$strategy->shouldCache()) {
            return $this->model->where('reviewsystem_id', $id)->first();
        }
        ...
        return $this->cache->remember(
            ...
            fn() => $this->model->where('reviewsystem_id', $id)->first()
        );
    }
    ...
    public function findByIdWithSeo(int $id): ?ReviewSystem
    {
        ...
        if ($strategy === CacheStrategy::ADMIN_FRESH) {
            return $this->model->with('seoSetting')->where('reviewsystem_id', $id)->first();
        }
        ...
        return $this->cache->remember(
            ...
            $strategy->getCacheTtl(),
            fn() => $this->model->with('seoSetting')->where('reviewsystem_id', $id)->first()
        );
    }
    ...
    public function getActive(): Collection
    {
        ...
        if (!$strategy->shouldCache()) {
            return $this->model->active()->orderBy('reviewsystem_id', 'desc')->get();
        }

        $cacheKey = $this->getCacheKey('active_reviewsystems');
        ...
        return Cache::tags($this->getCacheTags())
            ->remember(
                ...
                $strategy->getCacheTtl(),
                fn() =>
                $this->model->active()->orderBy('reviewsystem_id', 'desc')->get()
            );
    }
    ...
    public function getPaginated(array $filters = [], int $perPage = 10): LengthAwarePaginator
    {
        ...
        // Sorting
        $sortField = $filters['sortField'] ?? 'reviewsystem_id';
    ...
    {
        $reviewsystem = $this->model->create($data);
        $this->clearCache();
    ...
    {
        $result = $this->model->where('reviewsystem_id', $id)->update($data);
    ...
    {
        $result = $this->model->where('reviewsystem_id', $id)->delete();
    ...
    public function toggleActive(int $id): bool
    {
        ...
        $reviewsystem = $this->model->where('reviewsystem_id', $id)->first(['reviewsystem_id', 'is_active']);

        if (!$reviewsystem) {
            return false;
        }

        $result = $this->model->where('reviewsystem_id', $id)->update(['is_active' => !$reviewsystem->is_active]);
    ...
    {
        $count = $this->model->whereIn('reviewsystem_id', $ids)->delete();
    ...
    public function bulkToggleActive(array $ids): int
    {
        ...
        $reviewsystems = $this->model->whereIn('reviewsystem_id', $ids)->get(['reviewsystem_id', 'is_active']);
        $count = 0;

        foreach ($reviewsystems as $reviewsystem) {
            $this->model->where('reviewsystem_id', $reviewsystem->reviewsystem_id)
                ->update(['is_active' => !$reviewsystem->is_active]);
            $count++;
        }
    ...
    {
        $reviewsystem = $this->model->where('reviewsystem_id', $id)->first(['reviewsystem_id', 'seo']);

        if (!$reviewsystem) {
            return false;
        }

        $seo = $reviewsystem->seo ?? [];
        $seo[$locale][$field] = $value;

        $result = $this->model->where('reviewsystem_id', $id)->update(['seo' => $seo]);
