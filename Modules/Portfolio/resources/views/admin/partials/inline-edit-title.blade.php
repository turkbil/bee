<td wire:key="title-{{ $portfolio->portfolio_id }}" class="position-relative">
    ...
    <div class="d-flex align-items-center gap-3" x-data @click.outside="$wire.updateTitleInline()">
        <div class="flexible-input-wrapper">
            <input type="text" wire:model.defer="newTitle" class="form-control form-control-sm flexible-input"
    ...
    <div class="d-flex align-items-center">
        <span class="editable-title pr-4">{{ $portfolio->getTranslated('title') }}</span>
        <button class="btn btn-sm px-2 py-1 edit-icon ms-4"
            wire:click="startEditingTitle({{ $portfolio->portfolio_id }}, '{{ $portfolio->getTranslated('title') }}')">
