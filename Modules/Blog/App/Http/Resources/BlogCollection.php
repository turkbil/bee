...
/**
 * Blog Collection Resource
 *
 * Transforms collection of blogs for API responses.
...
class BlogCollection extends ResourceCollection
{
    ...
    private function getMeta(Request $request): array
    {
        ...
        if ($this->resource instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            $meta['pagination'] = [
                ...
                'per_blog' => $this->resource->perPage(),
                'current_blog' => $this->resource->currentPage(),
                'total_blogs' => $this->resource->lastPage(),
            ];
        } else {
    ...
    {
        // Add cache headers for list blogs
        if (!config('app.debug')) {
            $ttl = config('blog.cache.ttl.list', 3600);
