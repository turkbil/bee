...
const StudioToolbarManager = (function() {
    ...
    function setupDeviceButtons() {
        ...
        if (deviceButtons.desktop) {
            deviceButtons.desktop.addEventListener('click', function(e) {
                e.preventDefault();
        ...
        if (deviceButtons.tablet) {
            deviceButtons.tablet.addEventListener('click', function(e) {
                e.preventDefault();
                clearActiveDeviceButtons();
                this.classList.add('active');
        ...
        if (deviceButtons.mobile) {
            deviceButtons.mobile.addEventListener('click', function(e) {
                e.preventDefault();
                clearActiveDeviceButtons();
                this.classList.add('active');
    ...
    function handleDeviceSwitch(device) {
        // Canvas frame'ini güncelle
        const frame = editor.Canvas.getFrame();
        if (!frame) return;
        
        // Seçilen cihaza göre frame stilini güncelle
        switch (device) {
            case 'desktop':
                frame.set('width', '100%');
                break;
            case 'tablet':
                frame.set('width', '768px');
                break;
            case 'mobile':
                frame.set('width', '375px');
                break;
        }
