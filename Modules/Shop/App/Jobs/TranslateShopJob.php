...
/**
 * 🚀 Shop Translation Queue Job
 *
 * Bu job Shop modülündeki sayfaları AI ile çevirir:
 * - Toplu çeviri işlemleri için optimize edilmiş
 * - Progress tracking ile durum takibi
 * - Token kullanımı hesaplama
 */
class TranslateShopJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        try {
            ...
            $totalOperations = count($this->productIds) * count($targetLanguages);
            $processedCount = 0;
            $successCount = 0;
            $failedCount = 0;
            $totalTokensUsed = 0;
            ...
            foreach ($this->productIds as $productId) {
                ...
                if (!$product) {
                    Log::warning("Shop not found: {$productId}");
                    $failedCount++;
                    continue;
                }
                ...
                foreach ($this->targetLanguages as $targetLanguage) {
                    ...
                    try {
                        // Kaynak içeriği al
                        $title = is_array($product->title) ? ($product->title[$this->sourceLanguage] ?? '') : ($product->title ?? '');
                        $body = is_array($product->body) ? ($product->body[$this->sourceLanguage] ?? '') : ($product->body ?? '');
                        ...
                        if (empty(trim($sourceData['title'] . $sourceData['body']))) {
                            Log::info("Empty content for product {$productId}, language {$this->sourceLanguage}");
                            $processedCount++;
                            continue;
                        }
                        ...
                        if ($response['success'] ?? false) {
                            ...
                            // Sayfayı güncelle
                            $this->updatePageTranslation($product, $translatedData, $targetLanguage);
                            ...
                            try {
                                ...
                                ai_use_credits($perLanguageCost, $tenantId, [
                                    ...
                                    'description' => "Shop Translation: #{$productId} ({$this->sourceLanguage} → {$targetLanguage})",
                                    'entity_type' => 'product',
                                    'entity_id' => $productId,
                                    'source_language' => $this->sourceLanguage,
                                    'target_language' => $targetLanguage,
                                    'provider_name' => 'product_translation_service',
                                    'tokens_used' => $response['tokens_used'] ?? 0
                                ]);

                                Log::info('💰 KREDİ DÜŞÜRÜLDİ: SHOP ÇEVİRİ - 1 DİL = 1 KREDİ', [
                                    'product_id' => $productId,
                            ...
                            Log::info("Shop {$productId} translated to {$targetLanguage} successfully");
                        } else {
                            $failedCount++;
                            Log::error("Translation failed for product {$productId} to {$targetLanguage}: " . ($response['error'] ?? 'Unknown error'));
                        }
                    } catch (Throwable $e) {
                        $failedCount++;
                        Log::error("Translation error for product {$productId} to {$targetLanguage}: " . $e->getMessage() . "\nStack trace: " . $e->getTraceAsString());
                    }
            ...
            event(new \Modules\Shop\App\Events\TranslationCompletedEvent([
     ...
     */
    private function updatePageTranslation(Shop $product, array $translatedData, string $targetLanguage): void
    {
        // Mevcut çevirileri al
        $currentTitle = $product->title ?? [];
        $currentBody = $product->body ?? [];
        $currentExcerpt = $product->excerpt ?? [];
        ...
        $slug = [];
        if (is_array($product->slug)) {
            $slug = $product->slug;
        } elseif (is_string($product->slug)) {
    ...
    public function failed(Throwable $exception): void
    {
        ...
        Log::error("TranslateShopJob failed: " . $exception->getMessage());
    }
}
