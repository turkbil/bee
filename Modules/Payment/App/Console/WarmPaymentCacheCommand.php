...
/**
 * Warm Payment Cache Command
 *
 * Pre-loads frequently accessed payments into cache for better performance.
 * Supports multi-tenant architecture with isolated cache warming.
 *
 * @package Modules\Payment\App\Console
 *
 ...
 */
class WarmPaymentCacheCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'payment:warm-cache
        {--tenant=* : Specific tenant IDs to warm cache for}
        {--payments=10 : Number of payments to warm per tenant}
     ...
     * @var string
     */
    protected $description = 'Warm payment caches for better performance';
     ...
     */
    public function handle(PaymentService $paymentService, TenantCacheService $cacheService): int
    {
        $startTime = microtime(true);

        if (!$this->option('silent')) {
            $this->info('🔥 Starting Payment Cache Warming...');
            $this->newLine();
        }
        ...
        try {
            ...
            foreach ($tenantIds as $tenantId) {
                $this->processTenant($tenantId, $paymentService, $cacheService);
            }
            ...
            Log::error('Payment cache warming failed', [
                'error' => $e->getMessage(),
     ...
     */
    private function processTenant(int $tenantId, PaymentService $paymentService, TenantCacheService $cacheService): void
    {
        if (!$this->option('silent')) {
        ...
        // Get payments to warm
        $limit = (int) $this->option('payments');
        $payments = $this->getPagesToWarm($limit);

        if ($payments->isEmpty()) {
            if (!$this->option('silent')) {
                $this->warn("  No payments found for tenant #{$tenantId}");
            }
            return;
        }
        ...
        // Create progress bar
        if (!$this->option('silent')) {
            $this->progressBar = $this->output->createProgressBar($payments->count());
            $this->progressBar->setFormat(' %current%/%max% [%bar%] %percent:3s%% %message%');
        }

        // Warm cache for each payment
        foreach ($payments as $payment) {
            $this->warmPageCache($payment, $paymentService, $cacheService);
    ...
    /**
     * Warm cache for a specific payment
     */
    private function warmPageCache(Payment $payment, PaymentService $paymentService, TenantCacheService $cacheService): void
    {
        try {
            ...
            if (!$this->option('silent') && $this->progressBar) {
                $title = is_array($payment->title) ? ($payment->title['tr'] ?? 'Untitled') : 'Untitled';
                $this->progressBar->setMessage("Caching: {$title}");
            }

            // Cache key for this payment
            $cacheKey = "payment_detail_{$payment->payment_id}";
            ...
            // Cache payment data
            $ttl = config('payment.cache.ttl.detail', 7200);
            $cacheService->remember(
                TenantCacheService::PREFIX_PAGES,
                $cacheKey,
                $ttl,
                fn() => $payment->load('seoSetting')->toArray()
            );

            $this->stats['payments_cached']++;

            // Cache different language versions
            if (is_array($payment->slug)) {
                foreach ($payment->slug as $locale => $slug) {
                    $localeCacheKey = "payment_slug_{$locale}_{$slug}";
                    $cacheService->remember(
                        TenantCacheService::PREFIX_PAGES,
                        $localeCacheKey,
                        $ttl,
                        fn() => $payment->payment_id
                    );
                }
            }
            ...
            if ($this->option('urls')) {
                $this->warmPageUrls($payment);
            }

            // Cache SEO data separately (if SEO trait is available)
            if (method_exists($payment, 'hasSeoSettings') && $payment->hasSeoSettings()) {
                Cache::put(
                    "universal_seo_payment_{$payment->payment_id}",
                    method_exists($payment, 'getSeoData') ? $payment->getSeoData() : [],
                    $ttl
                );
            }
        } catch (\Exception $e) {
            ...
            Log::warning('Failed to warm cache for payment', [
     ...
     */
    private function warmPageUrls(Payment $payment): void
    {
        if (!is_array($payment->slug)) {
            return;
        }
        ...
        foreach ($payment->slug as $locale => $slug) {
            try {
                $url = route('payment.show', ['slug' => $slug, 'locale' => $locale]);
    ...
    private function displaySummary(): void
    {
        ...
        $this->table(
            ['Metric', 'Value'],
            [
                ['Pages Cached', $this->stats['payments_cached']],
    ...
    {
        $total = $this->stats['payments_cached'] + $this->stats['skipped'];
     ...
     * $schedule->command('payment:warm-cache --payments=20')
     *     ->hourly()
     *     ->between('08:00', '20:00')
     *     ->withoutOverlapping()
     *     ->runInBackground();
     ...
     * $schedule->command('payment:warm-cache --payments=50 --urls')
     *     ->dailyAt('03:00')
     *     ->withoutOverlapping()
     *     ->runInBackground();
     */
