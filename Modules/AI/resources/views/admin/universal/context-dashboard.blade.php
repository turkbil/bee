    ...
    return {
        ...
        generateHeatmap() {
            ...
            for (let day = 0; day < 7; day++) {
                for (let hour = 0; hour < 24; hour++) {
                    ...
                    // Simulate quality score (0-4)
                    const quality = Math.floor(Math.random() * 5);
                    cell.style.backgroundColor = colors[quality];
                    cell.title = `Day ${day + 1}, Hour ${hour}: Quality ${quality + 1}/5`;
        ...
        updateCharts() {
            if (window.contextChart) {
                window.contextChart.data.datasets[0].data = this.generateChartData();
                window.contextChart.data.datasets[1].data = this.generatePerformanceData();
                window.contextChart.update('active');
            }
        },
        ...
        exportChart() {
            if (window.contextChart) {
                const url = window.contextChart.toBase64Image();
                const link = document.createElement('a');
                link.download = 'context-analysis-chart.png';
                link.href = url;
                link.click();
            }
        },
        ...
        viewAlerts() {
            ...
            alert('Navigating to detailed alerts view...');
        },
        
        dismissAlerts() {
            this.hasActiveAlerts = false;
            this.metrics.alertsCount = 0;
        },
        
        implementRecommendation(recommendation) {
            alert(`Implementing recommendation: ${recommendation.title}`);
...
.context-timeline::-webkit-scrollbar {
    width: 6px;
}

.context-timeline::-webkit-scrollbar-track {
    background: #f1f5f9;
    border-radius: 3px;
}

.context-timeline::-webkit-scrollbar-thumb {
    background: #cbd5e1;
    border-radius: 3px;
}

.context-timeline::-webkit-scrollbar-thumb:hover {
    background: #94a3b8;
}

/* Pulse animation for live indicator */
.refresh-indicator .fas.fa-sync-alt:not(.fa-spin) {
