{{-- Modules/Blog/resources/views/admin/helper.blade.php --}}
{{-- PreTitle --}}
@section('pretitle')
    {{ __('blog::admin.blog_management') }}
@endsection

{{-- Başlık --}}
@section('title')
    {{ __('blog::admin.blogs') }}
@endsection

{{-- Modül Menüsü --}}
@push('module-menu')

    <div class="dropdown d-grid d-md-flex module-menu">
        <a href="#" class="btn dropdown-toggle d-inline-block d-lg-none"
            data-bs-toggle="dropdown">{{ __('blog::admin.menu') }}</a>
        <div class="dropdown-menu dropdown-module-menu">
            <div class="module-menu-revert">
                @hasmoduleaccess('blog', 'view')
                    <a href="{{ route('admin.blog.index') }}" class="dropdown-module-item btn btn-ghost-primary">
                        {{ __('blog::admin.blogs') }}
                    </a>
                @endhasmoduleaccess

                @hasmoduleaccess('blog', 'view')
                    <a href="{{ route('admin.blog.category.index') }}" class="dropdown-module-item btn btn-ghost-primary">
                        {{ __('blog::admin.categories') }}
                    </a>
                @endhasmoduleaccess

                @hasmoduleaccess('blog', 'create')
                    <a href="{{ route('admin.blog.manage') }}" class="dropdown-module-item btn btn-primary">
                        {{ __('blog::admin.new_blog') }}
                    </a>
                @endhasmoduleaccess
            </div>
        </div>
    </div>

@endpush
