<div>
    <!-- Mevcut Avatar -->
    <div class="mb-6">
        <h4 class="text-sm font-medium text-gray-700 mb-4">{{ __('usermanagement::admin.current_avatar') }}</h4>
        <div class="flex items-center space-x-4">
            @if($user->getFirstMedia('avatar'))
                ...
                <div class="text-sm text-gray-600">
                    <p class="font-medium">{{ __('usermanagement::admin.avatar_uploaded') }}</p>
                    <p>{{ __('usermanagement::admin.change_avatar_text') }}</p>
                </div>
            @else
                ...
                <div class="text-sm text-gray-600">
                    <p class="font-medium">{{ __('usermanagement::admin.no_avatar') }}</p>
                    <p>{{ __('usermanagement::admin.upload_profile_photo') }}</p>
                </div>
            @endif
        </div>
    </div>
    ...
    <form action="{{ route('avatar.upload') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="text-center">
                <label for="avatar-upload" class="cursor-pointer block">
                    ...
                    <p class="mb-2 text-sm text-gray-600">
                        <span class="font-semibold">{{ __('usermanagement::admin.click_or_select') }}</span>
                    </p>
                    <p class="text-xs text-gray-500">{{ __('usermanagement::admin.supported_formats') }}</p>
                </label>
        ...
        <button type="submit" class="mt-4 w-full bg-blue-500 hover:bg-blue-600 text-white font-medium py-2 px-4 rounded-lg transition-colors">
            {{ __('usermanagement::admin.avatar_upload') }}
        </button>
    </form>
