...
class StudioCacheService
{
    ...
    public function __construct()
    {
        ...
        $this->tenantId = function_exists('tenant') ? tenant()->getTenantKey() : 'central';
    }
    ...
    public function clear(): bool
    {
        try {
            ...
            if (config('cache.default') === 'redis') {
                ...
                $this->clearGenericCache($pattern);
            }
     ...
     * @return void
     */
    protected function clearGenericCache(string $pattern): void
    {
        // Pattern kullanılamıyorsa, tenant bazlı temizle
        Cache::flush();
    }
    ...
    public function clearByType(string $type): bool
    {
        try {
            ...
            if (config('cache.default') === 'redis') {
                ...
                switch ($type) {
                    case 'widget':
                        $this->forget('all_widgets');
                        $this->forget('widgets_as_blocks');
                        break;
                    case 'theme':
                        $this->forget('all_themes');
                        break;
                }
            }
