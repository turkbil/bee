...
namespace Modules\Service\App\Http\Controllers\Front;
...
class ServiceController extends Controller
{
    ...
    /**
     * Ana sayfa desteği yok - Service modülünde homeservice özelliği kaldırıldı
     */
    public function homeservice(SeoMetaTagService $seoService)
    {
        // Ana sayfa özelliği Service modülünde desteklenmiyor
        abort(404);
    }
    ...
    public function index()
    {
        ...
        try {
            ...
            return view('service::front.index', compact('items', 'moduleTitle'));
        }
    }
    ...
    public function show($slug, SeoMetaTagService $seoService = null)
    {
        ...
        if (config('service.debug.verbose_logs', false)) {
        ...
        $item = Service::where('is_active', true)
        ...
        if (!$item) {
            ...
            if (!$item) {
                Log::warning("Service not found in any language", [
                    'slug' => $slug,
                    'searched_locales' => $allLocales
                ]);
                abort(404, "Service not found for slug '{$slug}'");
            }
        }
        ...
        try {
            // Modül adıyla tema yolunu al
            $viewPath = $this->themeService->getThemeViewPath('show', 'service');
            ...
            // Fallback view'a yönlendir
            return view('service::front.show', ['item' => $item]);
        }
    }
     ...
     */
    protected function generatePageUrl(Service $service, ?string $locale = null): string
    {
        $locale = $locale ?? app()->getLocale();
        $slug = $service->getTranslated('slug', $locale);

        // Modül slug'ını al (tenant tarafından özelleştirilebilir)
        $moduleSlug = ModuleSlugService::getSlug('Service', 'show');
