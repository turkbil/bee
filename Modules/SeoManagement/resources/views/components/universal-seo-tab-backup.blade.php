    ...
    <div class="mt-4">
        ...
        <div>
            @php
                ...
                // ========================================
                // ANLIZ SİSTEMİ - CANLI HESAPLAMA
                // ========================================

                // 1. META TITLE ANALİZİ
                $titleToAnalyze = !empty($metaTitle) ? $metaTitle : $pageTitle;
                $titleLength = strlen($titleToAnalyze);
                $titleScore = 0;
                $titleStatus = 'danger';
                $titleMessage = 'Başlık yok';

                if (!empty($titleToAnalyze)) {
                    $titleScore += 30; // Var olması için 30 puan
                    $titleMessage = 'Başlık mevcut';
                    $titleStatus = 'warning';

                    if ($titleLength >= 30 && $titleLength <= 60) {
                        $titleScore += 50; // İdeal uzunluk için 50 puan
                        $titleMessage = 'İdeal uzunluk';
                        $titleStatus = 'success';
                    } elseif ($titleLength >= 20 && $titleLength < 30) {
                        $titleScore += 20; // Kısa ama kabul edilebilir
                        $titleMessage = 'Kısa ama kullanılabilir';
                    } elseif ($titleLength > 60) {
                        $titleScore += 10; // Uzun ama var
                        $titleMessage = 'Çok uzun - kesilecek';
                    }

                    if (!empty($metaTitle)) {
                        $titleScore += 20; // Özel meta title için bonus
                    }
                }

                // 2. META DESCRIPTION ANALİZİ
                $descLength = strlen($metaDescription);
                $descScore = 0;
                $descStatus = 'danger';
                $descMessage = 'Meta açıklama yok';

                if (!empty($metaDescription)) {
                    $descScore += 40; // Var olması için 40 puan
                    $descMessage = 'Meta açıklama mevcut';
                    $descStatus = 'warning';

                    if ($descLength >= 120 && $descLength <= 160) {
                        $descScore += 60; // İdeal uzunluk için 60 puan
                        $descMessage = 'İdeal uzunluk';
                        $descStatus = 'success';
                    } elseif ($descLength >= 80 && $descLength < 180) {
                        $descScore += 30; // Kabul edilebilir uzunluk
                        $descMessage = 'Kabul edilebilir uzunluk';
                    }
                }
                ...
                $contentMessage = 'İçerik eksik';

                if ($wordCount >= 300) {
                    $contentScore += 40;
                    $contentMessage = 'Yeterli içerik';
                    $contentStatus = 'success';
                } elseif ($wordCount >= 150) {
                    $contentScore += 20;
                    $contentMessage = 'Orta uzunluk içerik';
                    $contentStatus = 'warning';
                } elseif ($wordCount > 0) {
                    $contentScore += 10;
                    $contentMessage = 'Az içerik var';
                    $contentStatus = 'warning';
                }

                if ($hasH1) $contentScore += 20;
                if ($hasH2) $contentScore += 15;
                if ($linkCount > 0) $contentScore += 15;
                if ($wordCount >= 500) $contentScore += 10;

                // 4. SOSYAL MEDYA ANALİZİ
                $socialScore = 0;
                $socialStatus = 'danger';
                $socialMessage = 'Sosyal medya hazır değil';

                if (!empty($ogTitle) || !empty($metaTitle)) $socialScore += 25;
                if (!empty($ogDescription) || !empty($metaDescription)) $socialScore += 25;
                if (!empty($ogImage)) $socialScore += 30;
                if (!empty($authorName)) $socialScore += 20;

                if ($socialScore >= 70) {
                    $socialStatus = 'success';
                    $socialMessage = 'Sosyal medya hazır';
                } elseif ($socialScore >= 40) {
                    $socialStatus = 'warning';
                    $socialMessage = 'Kısmen hazır';
                }

                // 5. GENEL SKOR HESAPLAMA
                $overallScore = round(($titleScore + $descScore + $contentScore + $socialScore) / 4);
                $overallStatus = $overallScore >= 80 ? 'success' : ($overallScore >= 60 ? 'warning' : 'danger');
                $overallMessage = $overallScore >= 80 ? 'Mükemmel' : ($overallScore >= 60 ? 'İyi' : 'Geliştirilebilir');
            @endphp
