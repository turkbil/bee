...
#[Layout('admin.layout')]
class MuzibuManageComponent extends Component implements AIContentGeneratable
{
    ...
    // SOLID Dependencies
    protected $muzibuService;

    /**
     * Get current muzibu model
     */
    #[Computed]
    public function currentPage()
    {
        ...
        return Muzibu::query()->find($this->muzibuId);
    }
    ...
    {
        return \Modules\Muzibu\App\Models\MuzibuCategory::active()
            ->orderBy('sort_order')
            ->get();
    }
    ...
    {
        // MuzibuService'i initialize et (her zaman var)
        $this->muzibuService = app(\Modules\Muzibu\App\Services\MuzibuService::class);

        // Layout sections
        view()->share('pretitle', __('muzibu::admin.muzibu_management'));
        view()->share('title', __('muzibu::admin.muzibus'));
    }
    ...
    public function mount($id = null)
    {
        ...
        $studioConfig = config('muzibu.integrations.studio', []);
    ...
    protected function initializeUniversalComponents()
    {
        ...
        $this->activeTab = \App\Services\GlobalTabService::getDefaultTabKey('muzibu');
    }
    ...
    {
        // MuzibuService her zaman var, fallback gereksiz
        $formData = $this->muzibuService->prepareMuzibuForForm($id, $this->currentLanguage);
        $muzibu = $formData['muzibu'] ?? null;
        $this->tabCompletionStatus = $formData['tabCompletion'] ?? [];

        if ($muzibu) {
            // Dil-neutral alanlar
            $this->inputs = $muzibu->only(['is_active', 'muzibu_category_id']);

            // Çoklu dil alanları - FALLBACK KAPALI (kullanıcı tüm dilleri boşaltabilsin)
            foreach ($this->availableLanguages as $lang) {
                $this->multiLangInputs[$lang] = [
                    'title' => $muzibu->getTranslated('title', $lang, false) ?? '',
                    'body' => $muzibu->getTranslated('body', $lang, false) ?? '',
                    'slug' => $muzibu->getTranslated('slug', $lang, false) ?? '',
                ];
            }
    ...
    protected function rules()
    {
        $rules = [
            ...
            'inputs.muzibu_category_id' => 'nullable|exists:muzibu_categories,category_id',
        ];
    ...
    public function save($redirect = false, $resetForm = false)
    {
        ...
        $isNewRecord = !$this->muzibuId;

        if ($this->muzibuId) {
            $muzibu = Muzibu::query()->findOrFail($this->muzibuId);
            $currentData = collect($muzibu->toArray())->only(array_keys($data))->all();

            if ($data == $currentData) {
                $toast = [
                    'title' => __('admin.success'),
                    'message' => __('admin.muzibu_updated'),
                    'type' => 'success'
                ];
            } else {
                $muzibu->update($data);
            ...
            $muzibu = Muzibu::query()->create($data);
            $this->muzibuId = $muzibu->muzibu_id;
            log_activity($muzibu, 'eklendi');

            $toast = [
                'title' => __('admin.success'),
                'message' => __('admin.muzibu_created'),
                'type' => 'success'
            ];
        }
        ...
        $this->dispatch('page-saved', $this->muzibuId);

        // Redirect istendiyse
        if ($redirect) {
            session()->flash('toast', $toast);
            return redirect()->route('admin.muzibu.index');
        }

        // Yeni kayıt oluşturulduysa - medya event'ini dispatch et ve redirect
        if ($isNewRecord && isset($muzibu)) {
            // UniversalMediaComponent'e save event'i gönder
            $this->dispatch('muzibu-saved', $muzibu->muzibu_id);

            session()->flash('toast', $toast);
            return redirect()->route('admin.muzibu.manage', ['id' => $muzibu->muzibu_id]);
        }
    ...
    {
        return view('muzibu::admin.livewire.muzibu-manage-component', [
            'jsVariables' => [
                'currentMuzibuId' => $this->muzibuId ?? null,
    ...
    public function getTargetFields(array $params): array
    {
        ...
        if (isset($params['target_field'])) {
            return [$params['target_field'] => $muzibuFields[$params['target_field']] ?? 'html'];
        }
