...
namespace Modules\Payment\App\Http\Controllers\Front;
...
class PaymentController extends Controller
{
    ...
    /**
     * Ana sayfa desteği yok - Payment modülünde homepayment özelliği kaldırıldı
     */
    public function homepayment(SeoMetaTagService $seoService)
    {
        // Ana sayfa özelliği Payment modülünde desteklenmiyor
        abort(404);
    }
    ...
    public function index()
    {
        ...
        // Modül title'ını al
        $moduleTitle = $this->getModuleTitle('Payment');

        try {
            // Modül adıyla tema yolunu al
            $viewPath = $this->themeService->getThemeViewPath('index', 'payment');
            ...
            return view('payment::front.index', compact('items', 'moduleTitle'));
        }
    }
    ...
    public function show($slug, SeoMetaTagService $seoService = null)
    {
        ...
        if (config('payment.debug.verbose_logs', false)) {
        ...
        $item = Payment::where('is_active', true)
        ...
        if (!$item) {
            ...
            if (!$item) {
                Log::warning("Payment not found in any language", [
                    'slug' => $slug,
                    'searched_locales' => $allLocales
                ]);
                abort(404, "Payment not found for slug '{$slug}'");
            }
        }
        ...
        try {
            // Modül adıyla tema yolunu al
            $viewPath = $this->themeService->getThemeViewPath('show', 'payment');
            ...
            // Fallback view'a yönlendir
            return view('payment::front.show', ['item' => $item]);
        }
    }
     ...
     */
    protected function generatePageUrl(Payment $payment, ?string $locale = null): string
    {
        $locale = $locale ?? app()->getLocale();
        $slug = $payment->getTranslated('slug', $locale);

        // Modül slug'ını al (tenant tarafından özelleştirilebilir)
        $moduleSlug = ModuleSlugService::getSlug('Payment', 'show');
