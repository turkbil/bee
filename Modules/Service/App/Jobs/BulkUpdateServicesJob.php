...
/**
 * ✏️ Bulk Service Update Queue Job
 *
 * Service modülünün bulk güncelleme işlemleri için queue job:
 ...
 */
class BulkUpdateServicesJob implements ShouldQueue
{
    ...
    /**
     * @param array $serviceIds Güncellenecek sayfa ID'leri
    ...
    public function handle(ServiceService $serviceService): void
    {
        ...
        try {
            Log::info('✏️ BULK SERVICE UPDATE STARTED', [
            ...
            $progressKey = "bulk_update_services_{$this->tenantId}_{$this->userId}";
            ...
            foreach ($this->serviceIds as $index => $serviceId) {
                try {
                    ...
                    if ($this->options['validate'] ?? true) {
                        $this->validateUpdateData($service, $filteredUpdateData);
                    }

                    // Güncelleme işlemi
                    $service->update($filteredUpdateData);
                    log_activity($service, 'toplu-güncellendi');
                    ...
                    $errorCount++;
                    $errors[] = "Sayfa güncelleme hatası (ID: {$serviceId}): " . $e->getMessage();
            ...
            Log::error('💥 BULK SERVICE UPDATE FAILED', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
     ...
     */
    private function validateUpdateData(Service $service, array $updateData): void
    {
        // Slug benzersizlik kontrolü
        if (isset($updateData['slug'])) {
            $existingPage = Service::where('slug', $updateData['slug'])
                ->where('id', '!=', $service->id)
                ->first();
    ...
    private function clearPageCaches(): void
    {
        try {
            ...
            Log::info('🗑️ Service caches cleared after bulk update');
        } catch (\Exception $e) {
    ...
    {
        Log::error('💥 BULK SERVICE UPDATE JOB FAILED', [
            'service_ids' => $this->serviceIds,
            'update_data' => $this->updateData,
            'tenant_id' => $this->tenantId,
            'user_id' => $this->userId,
