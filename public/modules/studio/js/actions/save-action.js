...
const StudioSaveAction = (function() {
    ...
    function init(editorInstance, options = {}) {
        ...
        if (config.autoSaveInterval > 0) {
            setInterval(() => {
                ...
                if (editor.getDirtyCount && editor.getDirtyCount() > 0 && !isSaving) {
                    saveContent(true);
                }
    ...
    function saveContent(isAuto = false) {
        ...
        // İçeriği doğrudan al - HTML temizleme kaldırıldı
        ...
        // İçeriği doğrula
        if (!html || html.trim() === '') {
            showError('İçerik boş olamaz. Lütfen bir içerik ekleyin.');
            resetSaveButton();
            return;
        }
        ...
        .then(data => {
            if (data.success) {
                ...
                // Formu temizle - setDirtyCount fonksiyonunu güvenli bir şekilde çağır
                if (editor.setDirtyCount) {
