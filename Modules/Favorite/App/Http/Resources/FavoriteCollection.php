...
/**
 * Favorite Collection Resource
 *
 * Transforms collection of favorites for API responses.
...
class FavoriteCollection extends ResourceCollection
{
    ...
    private function getMeta(Request $request): array
    {
        ...
        if ($this->resource instanceof \Illuminate\Pagination\LengthAwarePaginator) {
            $meta['pagination'] = [
                ...
                'per_favorite' => $this->resource->perPage(),
                'current_favorite' => $this->resource->currentPage(),
                'total_favorites' => $this->resource->lastPage(),
            ];
        } else {
    ...
    {
        // Add cache headers for list favorites
        if (!config('app.debug')) {
            $ttl = config('favorite.cache.ttl.list', 3600);
