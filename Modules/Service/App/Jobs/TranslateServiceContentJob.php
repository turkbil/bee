...
class TranslateServiceContentJob implements ShouldQueue
{
     ...
     */
    public function __construct(array $data, int $serviceId)
    {
        $this->data = $data;
        $this->serviceId = $serviceId;
        $this->progressKey = "service_translation_progress_{$serviceId}_" . uniqid();
    ...
    public function handle(): void
    {
        try {
            ...
            Log::info("🔄 QUEUE SERVICE ÇEVİRİ BAŞLADI", [
            ...
            foreach ($targetLanguages as $targetLanguage) {
                ...
                try {
                    foreach ($fields as $field) {
                        $sourceText = $service->getTranslated($field, $sourceLanguage);
                        ...
                        $existingTranslation = $service->getTranslated($field, $targetLanguage);
                        ...
                        if (!empty($translatedText) && $translatedText !== $sourceText) {
                            ...
                            // Güncelle
                            $service->update([$field => $currentData]);
            ...
            if ($translatedCount > 0) {
                ...
                Log::info("🎉 QUEUE SERVICE ÇEVİRİ TAMAMLANDI", [
                    'service_id' => $this->serviceId,
                    'translated_count' => $translatedCount,
                    'messages' => $messages
                ]);
            ...
            Log::error("💥 QUEUE SERVICE ÇEVİRİ GENEL HATA", [
                'service_id' => $this->serviceId,
                'error' => $e->getMessage(),
    ...
    {
        Log::error("❌ QUEUE SERVICE ÇEVİRİ JOB BAŞARISIZ", [
            'service_id' => $this->serviceId,
