...
/**
 * Muzibu Bulk Operations Tests
 *
 * Toplu işlemlerin (bulk operations) doğru çalışmasını
 */
class MuzibuBulkOperationsTest extends TestCase
{
    use RefreshDatabase;
    ...
    /** @test */
    public function it_can_bulk_delete_multiple_muzibus(): void
    {
    ...
    /** @test */
    public function it_can_bulk_toggle_muzibu_status(): void
    {
        $activePages = Muzibu::factory()->active()->count(3)->create();
        $inactivePages = Muzibu::factory()->inactive()->count(2)->create();

        $ids = $activePages->pluck('muzibu_id')->toArray();
        ...
        foreach ($activePages as $muzibu) {
            $this->assertFalse($muzibu->fresh()->is_active);
        }
    }

    /** @test */
    public function it_can_toggle_both_active_and_inactive_muzibus(): void
    {
        $mixedPages = collect([
        ...
        $ids = $mixedPages->pluck('muzibu_id')->toArray();
    ...
    {
        $muzibu = Muzibu::factory()->create();
        $muzibu->getOrCreateSeoSetting(); // SEO ayarı ekle

        $this->assertDatabaseHas('seo_settings', [
            'seo_settingable_id' => $muzibu->muzibu_id
        ]);

        $this->service->bulkDeletePages([$muzibu->muzibu_id]);
    ...
    public function bulk_operations_return_correct_result_objects(): void
    {
        ...
        $this->assertInstanceOf(\Modules\Muzibu\App\DataTransferObjects\BulkOperationResult::class, $result);
        $this->assertTrue($result->success);
    ...
    public function partial_bulk_delete_returns_partial_result(): void
    {
        ...
        $muzibus = Muzibu::factory()->count(2)->create();

        $allIds = array_merge([$homemuzibu->muzibu_id], $muzibus->pluck('muzibu_id')->toArray());
    ...
    {
        $muzibus = Muzibu::factory()->count(100)->create();
        $ids = $muzibus->pluck('muzibu_id')->toArray();
    ...
    public function bulk_toggle_maintains_data_integrity(): void
    {
        ...
        foreach ($muzibus as $muzibu) {
            ...
            $this->assertEquals($muzibu->getTranslated('title', 'tr'), $fresh->getTranslated('title', 'tr'));
        }
    }
