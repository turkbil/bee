...
/**
 * Payment Category Seeder
 *
 * Temel payment kategorilerini oluşturur.
 * Tüm kategoriler JSON çoklu dil desteği ile gelir.
 *
 * @package Modules\Payment\Database\Seeders
 */
class PaymentCategorySeeder extends Seeder
{
    ...
    {
        // Payment kategorileri SADECE tenant database'lerde olmalı
        if (\App\Helpers\TenantHelpers::isCentral()) {
            $this->command->info('📁 Payment categories: sadece tenant database için, atlanıyor...');
            return;
        }
        ...
        if (!tenancy()->initialized) {
            $this->command->error('❌ Tenant context not initialized for Payment Categories!');
            return;
        }

        // Duplicate check
        if (PaymentCategory::count() > 3) {
            $this->command->warn("⚠️  Payment categories exist (>3). Skipping...");
            return;
        }
        ...
        $this->command->info("📁 Creating payment categories for languages: " . implode(', ', $this->languages));
        ...
        foreach ($categories as $categoryData) {
            ...
            $category = PaymentCategory::create($categoryData);
    ...
    private function createSeoSettings($category, $seoMeta): void
    {
        foreach ($seoMeta as $lang => $data) {
            $category->seoSetting()->updateOrCreate(
                ['seoable_id' => $category->category_id, 'seoable_type' => PaymentCategory::class],
                [
