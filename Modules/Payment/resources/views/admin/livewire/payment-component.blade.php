...
<div class="payment-component-wrapper">
    <div class="card">
                ...
                <div class="col">
                    <div class="input-icon">
                        ...
                        <input type="text" wire:model.live="search" class="form-control"
                            placeholder="{{ __('payment::admin.search_placeholder') }}">
                    </div>
                </div>
                <!-- Ortadaki Loading -->
                ...
                <table class="table table-vcenter card-table table-hover text-nowrap datatable">
                            ...
                            <th style="width: 50px">
                                <div class="d-flex align-items-center gap-2">
                                    <input type="checkbox"
                                           ...
                                           @checked($selectAll)>
                                    <button
                                        class="table-sort {{ $sortField === 'payment_id' ? ($sortDirection === 'asc' ? 'asc' : 'desc') : '' }}"
                                        wire:click="sortBy('payment_id')">
                                    </button>
                                </div>
                            </th>
                            <th>
                    ...
                    <tbody class="table-tbody">
                        @forelse($payments as $payment)
                            <tr class="hover-trigger" wire:key="row-{{ $payment->payment_id }}">
                                <td class="sort-id small">
                                    <div class="hover-toggle">
                                        <span class="hover-hide">{{ $payment->payment_id }}</span>
                                        <input type="checkbox"
                                               wire:model.live="selectedItems"
                                               value="{{ $payment->payment_id }}"
                                               class="form-check-input hover-show"
                                               id="checkbox-{{ $payment->payment_id }}"
                                               @checked(in_array($payment->payment_id, $selectedItems))>
                                    </div>
                                </td>
                                <td wire:key="title-{{ $payment->payment_id }}" class="position-relative">
                                    @if ($editingTitleId === $payment->payment_id)
                                    ...
                                    @else
                                        <div class="d-flex align-items-center">
                                            <span
                                                class="editable-title pr-4">{{ $payment->getTranslated('title', $currentSiteLocale) ?? $payment->getTranslated('title', 'tr') }}</span>
                                            <button class="btn btn-sm px-2 py-1 edit-icon ms-4"
                                                wire:click="startEditingTitle({{ $payment->payment_id }}, '{{ addslashes($payment->getTranslated('title', $currentSiteLocale) ?? $payment->getTranslated('title', 'tr')) }}')">
                                ...
                                <td class="text-center align-middle">
                                    <button wire:click="toggleActive({{ $payment->payment_id }})"
                                        class="btn btn-icon btn-sm {{ $payment->is_active ? 'text-muted bg-transparent' : 'text-red bg-transparent' }}">
                                        <!-- Loading Durumu -->
                                        <div wire:loading wire:target="toggleActive({{ $payment->payment_id }})"
                                            class="spinner-border spinner-border-sm">
                                        </div>
                                        <!-- Normal Durum: Aktif/Pasif İkonları -->
                                        <div wire:loading.remove
                                            wire:target="toggleActive({{ $payment->payment_id }})">
                                ...
                                <td class="text-center align-middle">
                                    <div class="d-flex align-items-center gap-3 justify-content-center">
                                        <a href="{{ route('admin.payment.manage', $payment->payment_id) }}"
                                            data-bs-toggle="tooltip" data-bs-placement="top"
                                            title="{{ __('admin.edit') }}"
                                            style="min-height: 24px; display: inline-flex; align-items: center; text-decoration: none;">
                                            <i class="fa-solid fa-pen-to-square link-secondary fa-lg"></i>
                                        </a>
                                        @if(config('payment.integrations.studio.enabled', false) && class_exists(config('payment.integrations.studio.component', '')))
                                        <a href="{{ route('admin.studio.editor', ['module' => 'payment', 'id' => $payment->payment_id]) }}"
                                            target="_blank" data-bs-toggle="tooltip" data-bs-placement="top"
                                            title="{{ __('payment::admin.studio.editor') }}"
                                            style="min-height: 24px; display: inline-flex; align-items: center; text-decoration: none;">
                                            <i class="fa-solid fa-wand-magic-sparkles link-secondary fa-lg"></i>
                                        </a>
                                        @endif
                                        <x-ai-translation :entity-type="'payment'" :entity-id="$payment->payment_id"
                                            tooltip="{{ __('admin.ai_translate') }}" />
                                        @hasmoduleaccess('payment', 'delete')
                                        <div class="dropdown">
                                            ...
                                            <div class="dropdown-menu dropdown-menu-end">
                                                <a href="javascript:void(0);"
                                                    ...
                                                    title: '{{ addslashes($payment->getTranslated('title', app()->getLocale()) ?? $payment->getTranslated('title', 'tr')) }}'
                                                })"
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="text-center py-4">
                                    <div class="empty">
                                        <p class="empty-title">{{ __('payment::admin.no_payments_found') }}</p>
                                        <p class="empty-subtitle text-muted">
                                            {{ __('payment::admin.no_results') }}
                                        </p>
                                    </div>
                                </td>
                            </tr>
            ...
            @else
                <div class="d-flex justify-content-between align-items-center mb-0">
                    <p class="small text-muted mb-0">
                        Toplam <span class="fw-semibold">{{ $payments->total() }}</span> sonuç
                    </p>
                </div>
            @endif
        </div>
        ...
        <!-- Bulk Actions -->
        @include('payment::admin.partials.bulk-actions', ['moduleType' => 'payment'])
