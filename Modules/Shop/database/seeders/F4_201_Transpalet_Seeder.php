...
class F4_201_Transpalet_Seeder extends Seeder
{
    public function run(): void
    {
        ...
        DB::table('shop_product_attributes')->whereIn('product_id', function($query) {
            $query->select('product_id')->from('shop_products')->where('sku', 'F4-201');
        })->delete();
        DB::table('shop_product_variants')->whereIn('product_id', function($query) {
            $query->select('product_id')->from('shop_products')->where('sku', 'F4-201');
        })->delete();
...
HTML,
        ...
        $features = [
            'tr' => [
                'list' => [
                    ...
                    'İXTİF stoktan hızlı teslim, yerinde montaj ve son kullanıcı desteği ile operasyona anında başlarsınız.',
        ...
        $faqData = [
            ...
            [
                'question' => [
                    'tr' => 'Standart çatal dışında hangi ölçüler mevcut?',
                    'en' => 'Standart çatal dışında hangi ölçüler mevcut?',
                    'vs.' => '...'
                ],
                'answer' => [
                    'tr' => 'Standart 1150 x 560 mm çatalın yanı sıra 900 mm’den 1500 mm’ye kadar uzanan uzunluk seçenekleri ve 560/685 mm açıklık kombinasyonları talebe göre sunulur.',
                    'en' => 'Standart 1150 x 560 mm çatalın yanı sıra 900 mm’den 1500 mm’ye kadar uzanan uzunluk seçenekleri ve 560/685 mm açıklık kombinasyonları talebe göre sunulur.',
                    'vs.' => '...'
                ],
                'sort_order' => 7
            ],
            [
                'question' => [
                    'tr' => 'İXTİF kiralık ve ikinci el seçeneklerini nasıl sunuyor?',
                    'en' => 'İXTİF kiralık ve ikinci el seçeneklerini nasıl sunuyor?',
                    'vs.' => '...'
                ],
                'answer' => [
                    'tr' => 'İXTİF, F4 201 için stoktan teslim ikinci el üniteler ve kısa/uzun dönem kiralama planları sunar; ihtiyaca göre bakım ve yedek parça paketi ile birlikte teklif edilir.',
                    'en' => 'İXTİF, F4 201 için stoktan teslim ikinci el üniteler ve kısa/uzun dönem kiralama planları sunar; ihtiyaca göre bakım ve yedek parça paketi ile birlikte teklif edilir.',
                    'vs.' => '...'
                ],
                'sort_order' => 8
            ],
            [
                'question' => [
                    'tr' => 'Saha kurulumu ve operatör eğitimi veriliyor mu?',
                    'en' => 'Saha kurulumu ve operatör eğitimi veriliyor mu?',
                    'vs.' => '...'
                ],
                'answer' => [
                    'tr' => 'İXTİF uzman ekibi cihazı sahada devreye alır, operatörlerin güvenli kullanımını ve hızlı adaptasyonunu sağlayacak eğitim setini yerinde verir.',
                    'en' => 'İXTİF uzman ekibi cihazı sahada devreye alır, operatörlerin güvenli kullanımını ve hızlı adaptasyonunu sağlayacak eğitim setini yerinde verir.',
                    'vs.' => '...'
                ],
                'sort_order' => 9
            ],
            [
                'question' => [
                    ...
                    'en' => 'Teknik slogan ve motto nedir?',
                    'vs.' => '...'
                ],
                'answer' => [
                    'tr' => 'Slogan: “Depoda hız, sahada prestij.” Motto: “İXTİF farkı ile 2 tonluk yükler bile hafifler.” Bu mesajlar satış sayfasında ayrı kartlarda öne çıkar.',
                    'en' => 'Slogan: “Depoda hız, sahada prestij.” Motto: “İXTİF farkı ile 2 tonluk yükler bile hafifler.” Bu mesajlar satış sayfasında ayrı kartlarda öne çıkar.',
                    'vs.' => '...'
                ],
                'sort_order' => 10
            ]
        ...
        $productId = DB::table('shop_products')->insertGetId([
            ...
            'brand_id' => 1,
            'sku' => $productData['sku'],
            'model_number' => $productData['model_code'],
            'barcode' => null,
            'title' => json_encode($productData['name'], JSON_UNESCAPED_UNICODE),
            'slug' => json_encode($productData['slug'], JSON_UNESCAPED_UNICODE),
            'short_description' => json_encode($productData['short_description'], JSON_UNESCAPED_UNICODE),
            'long_description' => json_encode($productData['long_description'], JSON_UNESCAPED_UNICODE),
            ...
            'technical_specs' => json_encode($productData['technical_specs'], JSON_UNESCAPED_UNICODE),
            ...
            'primary_specs' => json_encode($productData['primary_specs'], JSON_UNESCAPED_UNICODE),
            'use_cases' => json_encode($productData['use_cases'], JSON_UNESCAPED_UNICODE),
            'competitive_advantages' => json_encode($productData['competitive_advantages'], JSON_UNESCAPED_UNICODE),
            'target_industries' => json_encode($productData['target_industries'], JSON_UNESCAPED_UNICODE),
            'faq_data' => json_encode($productData['faq_data'], JSON_UNESCAPED_UNICODE),
            'video_url' => null,
            'manual_pdf_url' => null,
            'is_active' => $productData['status'] === 'active' ? 1 : 0,
        ...
        $this->command->info("✅ Ürün eklendi (ID: {$productId})");

        // Varyantları ekle
        foreach ($productData['variants'] as $variantData) {
            $variantId = DB::table('shop_product_variants')->insertGetId([
                'product_id' => $productId,
                'title' => json_encode([
                    'tr' => $variantData['variant_name'],
                    'en' => $variantData['variant_name'],
                    'vs.' => '...'
                ], JSON_UNESCAPED_UNICODE),
                'sku' => $variantData['sku'],
                'option_values' => json_encode($variantData['specifications'], JSON_UNESCAPED_UNICODE),
                'price_modifier' => 0,
                'cost_price' => null,
            ...
            $this->command->info("  ➕ Varyant: {$variantData['variant_name']} (ID: {$variantId})");
        }

        // Attribute'ları ekle (şimdilik devre dışı - attribute tablosu boş)
        $this->command->info("  ⚠️  Attribute'lar atlandı (shop_attributes tablosu boş)");
        // TODO: ShopAttributeSeeder çalıştırıldıktan sonra bu bölüm aktif edilecek

        $this->command->info('🎉 F4 201 Li-Ion Transpalet başarıyla eklendi!');
        $this->command->info('📊 İstatistik:');
        $this->command->info('  - Varyant sayısı: ' . count($productData['variants']));
        $this->command->info('  - Attribute sayısı: ' . count($productData['attributes']));
        $this->command->info('  - FAQ sayısı: ' . count($productData['faq_data']));
        $this->command->info('  - Use cases sayısı: ' . count($productData['use_cases']['tr']));
        $this->command->info('  - Competitive advantages sayısı: ' . count($productData['competitive_advantages']['tr']));
        $this->command->info('  - Target industries sayısı: ' . count($productData['target_industries']['tr']));
    }
}
