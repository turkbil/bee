    ...
    <article>
        <div class="relative">
            ...
            <div class="absolute top-4 right-4">
                <a href="{{ url('portfolio', 'category_slug', [$item->category->slug]) }}" class="text-primary dark:text-primary-400 hover:underline text-sm font-medium transition-colors">
                    {{ $item->category->title }}
                </a>
            </div>
            @endif
        ...
        <div class="py-6">
            ...
            <div class="flex flex-wrap items-center gap-3 mb-6 pb-4 border-b text-sm text-gray-600 dark:text-gray-400">
                ...
                <span class="flex items-center">
                    ...
                    <a href="{{ url('portfolio', 'category_slug', [$item->category->slug]) }}" class="hover:text-primary dark:hover:text-primary-400 transition-colors">
                        {{ $item->category->title }}
                    </a>
                </span>
                @endif
            ...
            <div class="mt-8 flex flex-wrap justify-between items-center gap-4">
                <a href="{{ url('portfolio', 'index_slug') }}" class="inline-flex items-center text-sm text-primary dark:text-primary-400 hover:underline font-medium">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                @if(isset($item->category))
                <a href="{{ url('portfolio', 'category_slug', [$item->category->slug]) }}" class="inline-flex items-center text-sm text-primary dark:text-primary-400 hover:underline font-medium">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l7 7a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-7-7A1.994 1.994 0 013 12V7a4 4 0 014-4z" />
                    </svg>
