...
use Modules\Payment\App\Services\PaymentCategoryService;
use Illuminate\Support\Facades\Log;
use App\Helpers\SlugHelper;

#[Layout('admin.layout')]
class PaymentCategoryManageComponent extends Component
{
    public $categoryId;
    ...
    public function currentCategory()
    {
        ...
        return PaymentCategory::query()->find($this->categoryId);
    }
    ...
    {
        $categories = PaymentCategory::orderBy('sort_order', 'asc')
    ...
    {
        $this->categoryService = app(\Modules\Payment\App\Services\PaymentCategoryService::class);

        view()->share('pretitle', __('payment::admin.category_management'));
        view()->share('title', __('payment::admin.categories'));
    }
    ...
    protected function initializeUniversalComponents()
    {
        ...
        $this->tabConfig = \App\Services\GlobalTabService::getAllTabs('payment_category');
        $this->activeTab = \App\Services\GlobalTabService::getDefaultTabKey('payment_category');
    }
    ...
    public function handleLanguageChange($language)
    {
        if (in_array($language, $this->availableLanguages)) {
            ...
            Log::info('PaymentCategoryManage - Dil değişti', [
    ...
    protected function rules()
    {
        $rules = [
            ...
            'inputs.parent_id' => 'nullable|exists:payment_categories,category_id',
        ];
    ...
    public function save($redirect = false, $resetForm = false)
    {
        ...
        if ($this->categoryId) {
            $category = PaymentCategory::query()->findOrFail($this->categoryId);
            $category->update($data);
            log_activity($category, 'güncellendi');

            $toast = [
                'title' => __('admin.success'),
                'message' => __('payment::admin.category_updated'),
                'type' => 'success'
            ];
        } else {
            $category = PaymentCategory::query()->create($data);
        ...
        if ($redirect) {
            ...
            return redirect()->route('admin.payment.category.index');
        }
    ...
    {
        return view('payment::admin.livewire.category-manage-component', [
            'jsVariables' => [
