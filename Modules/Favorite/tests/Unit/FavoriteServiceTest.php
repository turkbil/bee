<?php

declare(strict_types=1);

namespace Modules\Favorite\Tests\Unit;

use Modules\Favorite\Tests\TestCase;
use Modules\Favorite\App\Models\Favorite;
use Modules\Favorite\App\Services\FavoriteService;
use Modules\Favorite\App\Repositories\FavoriteRepository;
use Modules\Favorite\App\Contracts\FavoriteRepositoryInterface;
use App\Contracts\GlobalSeoRepositoryInterface;
use Modules\Favorite\App\Exceptions\{FavoriteNotFoundException, FavoriteProtectionException};
use Illuminate\Support\Facades\Log;

/**
 * FavoriteService Unit Tests
 *
 * Business logic katmanının tüm operasyonlarını,
 * exception handling ve slug generation gibi işlemleri test eder.
 */
class FavoriteServiceTest extends TestCase
{
    use RefreshDatabase;

    private FavoriteService $service;
    private FavoriteRepositoryInterface $repository;

    protected function setUp(): void
    {
        parent::setUp();

        $this->repository = app(FavoriteRepositoryInterface::class);
        $this->service = app(FavoriteService::class);

        Log::spy(); // Log facade'ini mock et
    }

    /** @test */
    public function it_can_get_favorite_by_id(): void
    {
        $favorite = Favorite::factory()->create();

        $result = $this->service->getPage($favorite->favorite_id);

        $this->assertInstanceOf(Favorite::class, $result);
        $this->assertEquals($favorite->favorite_id, $result->favorite_id);
    }

    /** @test */
    public function it_throws_exception_when_favorite_not_found(): void
    {
        $this->expectException(FavoriteNotFoundException::class);

        $this->service->getPage(999);
    }

    /** @test */
    public function it_can_get_favorite_by_slug(): void
    {
        $favorite = Favorite::factory()->active()->create([
            'slug' => ['tr' => 'test-slug', 'en' => 'test-slug']
        ]);

        $result = $this->service->getPageBySlug('test-slug', 'tr');

        $this->assertInstanceOf(Favorite::class, $result);
        $this->assertEquals($favorite->favorite_id, $result->favorite_id);
    }

    /** @test */
    public function it_throws_exception_when_slug_not_found(): void
    {
        $this->expectException(FavoriteNotFoundException::class);

        $this->service->getPageBySlug('nonexistent-slug', 'tr');
    }

    /** @test */
    public function it_can_get_active_favorites(): void
    {
        Favorite::factory()->active()->count(5)->create();
        Favorite::factory()->inactive()->count(3)->create();

        $result = $this->service->getActivePages();

        $this->assertCount(5, $result);
    }

    /** @test */

    /** @test */
    public function it_can_create_favorite_successfully(): void
    {
        $data = [
            'title' => ['tr' => 'Test Sayfası', 'en' => 'Test Favorite'],
            'body' => ['tr' => '<p>İçerik</p>', 'en' => '<p>Content</p>'],
            'is_active' => true,
        ];

        $result = $this->service->createPage($data);

        $this->assertTrue($result->success);
        $this->assertInstanceOf(Favorite::class, $result->data);
        $this->assertEquals('Test Sayfası', $result->data->getTranslated('title', 'tr'));
    }

    /** @test */
    public function it_generates_slug_automatically_when_creating_favorite(): void
    {
        $data = [
            'title' => ['tr' => 'Test Sayfası', 'en' => 'Test Favorite'],
            'body' => ['tr' => '<p>İçerik</p>', 'en' => '<p>Content</p>'],
            'is_active' => true,
        ];

        $result = $this->service->createPage($data);

        $this->assertEquals('test-sayfasi', $result->data->getTranslated('slug', 'tr'));
        $this->assertEquals('test-favorite', $result->data->getTranslated('slug', 'en'));
    }

    /** @test */
    public function it_logs_favorite_creation(): void
    {
        Log::shouldReceive('info')
            ->once()
            ->with('Favorite created', \Mockery::any());

        $data = [
            'title' => ['tr' => 'Test', 'en' => 'Test'],
            'body' => ['tr' => 'Test', 'en' => 'Test'],
            'is_active' => true,
        ];

        $this->service->createPage($data);
    }

    /** @test */
    public function it_can_update_favorite_successfully(): void
    {
        $favorite = Favorite::factory()->create([
            'title' => ['tr' => 'Eski Başlık', 'en' => 'Old Title']
        ]);

        $result = $this->service->updatePage($favorite->favorite_id, [
            'title' => ['tr' => 'Yeni Başlık', 'en' => 'New Title']
        ]);

        $this->assertTrue($result->success);
        $this->assertEquals('Yeni Başlık', $favorite->fresh()->getTranslated('title', 'tr'));
    }

    /** @test */
    public function it_throws_exception_when_updating_nonexistent_favorite(): void
    {
        $this->expectException(FavoriteNotFoundException::class);

        $this->service->updatePage(999, ['title' => ['tr' => 'Test']]);
    }

    /** @test */
    public function it_can_delete_regular_favorite(): void
    {
        $favorite = Favorite::factory()->create();

        $result = $this->service->deletePage($favorite->favorite_id);

        $this->assertTrue($result->success);
        $this->assertDatabaseMissing('favorites', ['favorite_id' => $favorite->favorite_id]);
    }

    /** @test */

    /** @test */

    /** @test */
    public function it_can_toggle_favorite_status(): void
    {
        $favorite = Favorite::factory()->active()->create();

        $result = $this->service->togglePageStatus($favorite->favorite_id);

        $this->assertTrue($result->success);
        $this->assertFalse($favorite->fresh()->is_active);
    }

    /** @test */

    /** @test */
    public function it_can_bulk_delete_favorites(): void
    {
        $favorites = Favorite::factory()->count(5)->create();
        $ids = $favorites->pluck('favorite_id')->toArray();

        $result = $this->service->bulkDeletePages($ids);

        $this->assertTrue($result->success);
        $this->assertEquals(5, $result->affectedCount);
    }

    /** @test */

    /** @test */
    public function it_returns_failure_when_no_favorites_can_be_deleted(): void
    {
        $homefavorite = Favorite::factory()->create();

        $result = $this->service->bulkDeletePages([$homefavorite->favorite_id]);

        $this->assertFalse($result->success);
    }

    /** @test */
    public function it_can_bulk_toggle_status(): void
    {
        $favorites = Favorite::factory()->active()->count(5)->create();
        $ids = $favorites->pluck('favorite_id')->toArray();

        $result = $this->service->bulkToggleStatus($ids);

        $this->assertTrue($result->success);
        $this->assertEquals(5, $result->affectedCount);
    }

    /** @test */
    public function it_can_search_favorites(): void
    {
        Favorite::factory()->active()->create([
            'title' => ['tr' => 'Laravel Öğreniyorum', 'en' => 'Learning Laravel']
        ]);
        Favorite::factory()->active()->create([
            'title' => ['tr' => 'PHP Temelleri', 'en' => 'PHP Basics']
        ]);

        $results = $this->service->searchPages('Laravel', ['tr', 'en']);

        $this->assertCount(1, $results);
    }

    /** @test */
    public function it_prepares_seo_data_correctly(): void
    {
        $data = [
            'title' => ['tr' => 'Test Sayfası', 'en' => 'Test Favorite'],
            'body' => ['tr' => '<p>Test</p>', 'en' => '<p>Test</p>'],
            'seo' => [
                'tr' => [
                    'meta_title' => 'SEO Başlık',
                    'meta_description' => 'SEO Açıklama',
                ],
                'en' => [
                    'meta_title' => 'SEO Title',
                    'meta_description' => 'SEO Description',
                ]
            ]
        ];

        $result = $this->service->createPage($data);

        $this->assertTrue($result->success);
    }

    /** @test */
    public function it_filters_empty_seo_values(): void
    {
        $data = [
            'title' => ['tr' => 'Test', 'en' => 'Test'],
            'body' => ['tr' => 'Test', 'en' => 'Test'],
            'seo' => [
                'tr' => [
                    'meta_title' => 'Başlık',
                    'meta_description' => '', // Boş değer
                    'meta_keywords' => null, // Null değer
                ]
            ]
        ];

        $result = $this->service->createPage($data);

        $this->assertTrue($result->success);
    }

    /** @test */
    public function it_can_prepare_favorite_for_form(): void
    {
        $favorite = Favorite::factory()->create();

        $formData = $this->service->preparePageForForm($favorite->favorite_id, 'tr');

        $this->assertArrayHasKey('favorite', $formData);
        $this->assertArrayHasKey('seoData', $formData);
        $this->assertArrayHasKey('tabCompletion', $formData);
        $this->assertArrayHasKey('tabConfig', $formData);
        $this->assertArrayHasKey('seoLimits', $formData);
    }

    /** @test */
    public function it_returns_empty_form_data_for_new_favorite(): void
    {
        $formData = $this->service->getEmptyFormData('tr');

        $this->assertNull($formData['favorite']);
        $this->assertArrayHasKey('seoData', $formData);
        $this->assertArrayHasKey('tabCompletion', $formData);
    }

    /** @test */
    public function it_provides_validation_rules(): void
    {
        $rules = $this->service->getValidationRules(['tr', 'en']);

        $this->assertArrayHasKey('inputs.css', $rules);
        $this->assertArrayHasKey('inputs.js', $rules);
        $this->assertArrayHasKey('multiLangInputs.tr.title', $rules);
        $this->assertArrayHasKey('multiLangInputs.en.title', $rules);
    }

    /** @test */
    public function it_clears_cache(): void
    {
        Log::shouldReceive('info')
            ->once()
            ->with('Favorite cache cleared', \Mockery::any());

        $this->service->clearCache();
    }

    /** @test */
    public function it_handles_slug_uniqueness_in_updates(): void
    {
        $existingPage = Favorite::factory()->create([
            'slug' => ['tr' => 'existing-slug', 'en' => 'existing-slug']
        ]);

        $newPage = Favorite::factory()->create([
            'slug' => ['tr' => 'new-slug', 'en' => 'new-slug']
        ]);

        // Existing slug'ı kullanmaya çalış - sistem otomatik unique yapmalı
        $result = $this->service->updatePage($newPage->favorite_id, [
            'title' => ['tr' => 'Yeni Başlık', 'en' => 'New Title']
        ]);

        $this->assertTrue($result->success);
    }

    /** @test */
    public function it_preserves_existing_slugs_when_updating(): void
    {
        $favorite = Favorite::factory()->create([
            'title' => ['tr' => 'Eski Başlık', 'en' => 'Old Title'],
            'slug' => ['tr' => 'custom-slug', 'en' => 'custom-slug']
        ]);

        $result = $this->service->updatePage($favorite->favorite_id, [
            'title' => ['tr' => 'Yeni Başlık', 'en' => 'New Title']
        ]);

        // Slug değişmeden kalmalı
        $this->assertEquals('custom-slug', $favorite->fresh()->getTranslated('slug', 'tr'));
    }
}
