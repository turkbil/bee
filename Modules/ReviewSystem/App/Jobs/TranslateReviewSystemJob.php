...
/**
 * 🚀 ReviewSystem Translation Queue Job
 *
 * Bu job ReviewSystem modülündeki sayfaları AI ile çevirir:
 * - Toplu çeviri işlemleri için optimize edilmiş
 * - Progress tracking ile durum takibi
 * - Token kullanımı hesaplama
 */
class TranslateReviewSystemJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        Log::info('🔥🔥🔥 TRANSLATEPAGEJOB HANDLE() BAŞLADI! 🔥🔥🔥', [
            'reviewsystemIds' => $this->reviewsystemIds,
        ...
        try {
            ...
            $totalOperations = count($this->reviewsystemIds) * count($targetLanguages);
            $processedCount = 0;
            $successCount = 0;
            $failedCount = 0;
            $totalTokensUsed = 0;

            foreach ($this->reviewsystemIds as $reviewsystemId) {
                $reviewsystem = ReviewSystem::find($reviewsystemId);

                if (!$reviewsystem) {
                    Log::warning("ReviewSystem not found: {$reviewsystemId}");
                    $failedCount++;
                    continue;
                }
                ...
                foreach ($this->targetLanguages as $targetLanguage) {
                    ...
                    try {
                        // Kaynak içeriği al
                        $title = is_array($reviewsystem->title) ? ($reviewsystem->title[$this->sourceLanguage] ?? '') : ($reviewsystem->title ?? '');
                        $body = is_array($reviewsystem->body) ? ($reviewsystem->body[$this->sourceLanguage] ?? '') : ($reviewsystem->body ?? '');
                        ...
                        if (empty(trim($sourceData['title'] . $sourceData['body']))) {
                            Log::info("Empty content for reviewsystem {$reviewsystemId}, language {$this->sourceLanguage}");
                            $processedCount++;
                            continue;
                        }
                        ...
                        if ($response['success'] ?? false) {
                            ...
                            // Sayfayı güncelle
                            $this->updatePageTranslation($reviewsystem, $translatedData, $targetLanguage);
                            ...
                            try {
                                ...
                                ai_use_credits($perLanguageCost, $tenantId, [
                                    ...
                                    'description' => "ReviewSystem Translation: #{$reviewsystemId} ({$this->sourceLanguage} → {$targetLanguage})",
                                    'entity_type' => 'reviewsystem',
                                    'entity_id' => $reviewsystemId,
                                    'source_language' => $this->sourceLanguage,
                                    'target_language' => $targetLanguage,
                                    'provider_name' => 'reviewsystem_translation_service',
                                    'tokens_used' => $response['tokens_used'] ?? 0
                                ]);

                                Log::info('💰 KREDİ DÜŞÜRÜLDİ: REVIEWSYSTEM ÇEVİRİ - 1 DİL = 1 KREDİ', [
                                    'reviewsystem_id' => $reviewsystemId,
                            ...
                            Log::info("ReviewSystem {$reviewsystemId} translated to {$targetLanguage} successfully");
                        } else {
                            $failedCount++;
                            Log::error("Translation failed for reviewsystem {$reviewsystemId} to {$targetLanguage}: " . ($response['error'] ?? 'Unknown error'));
                        }
                    } catch (Throwable $e) {
                        $failedCount++;
                        Log::error("Translation error for reviewsystem {$reviewsystemId} to {$targetLanguage}: " . $e->getMessage() . "\nStack trace: " . $e->getTraceAsString());
                    }
            ...
            event(new \Modules\ReviewSystem\App\Events\TranslationCompletedEvent([
     ...
     */
    private function updatePageTranslation(ReviewSystem $reviewsystem, array $translatedData, string $targetLanguage): void
    {
        // Mevcut çevirileri al
        $currentTitle = $reviewsystem->title ?? [];
        $currentBody = $reviewsystem->body ?? [];
        $currentExcerpt = $reviewsystem->excerpt ?? [];
        ...
        if (is_array($reviewsystem->slug)) {
            $slug = $reviewsystem->slug;
        } elseif (is_string($reviewsystem->slug)) {
            $slug = json_decode($reviewsystem->slug, true) ?? [];
        }
        ...
        log_activity($reviewsystem, 'çevrildi');
    }
    ...
    public function failed(Throwable $exception): void
    {
        ...
        Log::error("TranslateReviewSystemJob failed: " . $exception->getMessage());
    }
}
