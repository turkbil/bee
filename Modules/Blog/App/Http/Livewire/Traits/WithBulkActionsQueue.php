...
namespace Modules\Blog\App\Http\Livewire\Traits;
 ...
 *
 * Blog modülü için queue-based bulk işlemler:
 * - Bulk delete (queue)
 * - Bulk update (queue)
 * - Progress tracking
...
trait WithBulkActionsQueue
{
    ...
    public function bulkDelete()
    {
        ...
        try {
            ...
            \Modules\Blog\App\Jobs\BulkDeleteBlogsJob::dispatch(
                $this->selectedItems,
                $tenantId,
                $userId,
                ['force_delete' => false]
            ...
            $this->bulkProgress = [
                'operation' => 'delete',
                'count' => $count,
                'progress_key' => "bulk_delete_blogs_{$tenantId}_{$userId}",
    ...
    public function bulkUpdate($updateData)
    {
        ...
        try {
            ...
            \Modules\Blog\App\Jobs\BulkUpdateBlogsJob::dispatch(
                $this->selectedItems,
                $updateData,
                $tenantId,
                $userId,
            ...
            $this->bulkProgress = [
                'operation' => 'update',
                'count' => $count,
                'progress_key' => "bulk_update_blogs_{$tenantId}_{$userId}",
                'started_at' => now()->toISOString()
            ];
     ...
     */
    public function translateContent($data, ?int $blogId = null): void
    {
        if (!$blogId) {
            $this->dispatch('toast', [
                'title' => 'Çeviri Hatası',
        ...
        try {
            ...
            $progressKey = "blog_translation_progress_{$blogId}_" . uniqid();

            // Translation job dispatch et
            $job = \Modules\Blog\App\Jobs\TranslateBlogContentJob::dispatch($data, $blogId);
