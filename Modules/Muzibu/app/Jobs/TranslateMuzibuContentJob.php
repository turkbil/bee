...
class TranslateMuzibuContentJob implements ShouldQueue
{
    ...
    protected int $muzibuId;
    protected string $progressKey;

    /**
     * Create a new job instance.
     */
    public function __construct(array $data, int $muzibuId)
    {
        $this->data = $data;
        $this->muzibuId = $muzibuId;
    ...
    public function handle(): void
    {
        try {
            ...
            Log::info("🔄 QUEUE MUZIBU ÇEVİRİ BAŞLADI", [
                'muzibu_id' => $this->muzibuId,
            ...
            // Sayfayı bul
            $muzibu = Muzibu::findOrFail($this->muzibuId);
            ...
            foreach ($targetLanguages as $targetLanguage) {
                ...
                try {
                    foreach ($fields as $field) {
                        $sourceText = $muzibu->getTranslated($field, $sourceLanguage);
                        ...
                        $existingTranslation = $muzibu->getTranslated($field, $targetLanguage);
                        ...
                        if (!empty($translatedText) && $translatedText !== $sourceText) {
                            ...
                            // Güncelle
                            $muzibu->update([$field => $currentData]);
            ...
            if ($translatedCount > 0) {
                ...
                Log::info("🎉 QUEUE MUZIBU ÇEVİRİ TAMAMLANDI", [
                    'muzibu_id' => $this->muzibuId,
                    'translated_count' => $translatedCount,
                    'messages' => $messages
                ]);
            ...
            Log::error("💥 QUEUE MUZIBU ÇEVİRİ GENEL HATA", [
                'muzibu_id' => $this->muzibuId,
                'error' => $e->getMessage(),
    ...
    {
        Log::error("❌ QUEUE MUZIBU ÇEVİRİ JOB BAŞARISIZ", [
            'muzibu_id' => $this->muzibuId,
