...
namespace Modules\Muzibu\App\Http\Resources;
 ...
 * Muzibu Resource
 *
 * Transforms Muzibu model for API responses.
 * Implements JSON API specification.
 *
 * @property \Modules\Muzibu\App\Models\Muzibu $resource
 */
class MuzibuResource extends JsonResource
{
    /**
    ...
    private function getAttributes(string $locale, array $fields): array
    {
        $attributes = [
            ...
            'body' => $this->when(
                !isset($fields['muzibus']) || in_array('body', $fields['muzibus'] ?? []),
                fn() => $this->getTranslatedAttribute('body', $locale)
            ),
            'css' => $this->when(
                !isset($fields['muzibus']) || in_array('css', $fields['muzibus'] ?? []),
                $this->css
            ),
            'js' => $this->when(
                !isset($fields['muzibus']) || in_array('js', $fields['muzibus'] ?? []),
                $this->js
            ),
    ...
    private function getRelationships(array $includeRelations): array
    {
        ...
        if ($this->relationLoaded('seoSetting') || in_array('seo', $includeRelations)) {
            $relationships['seo'] = [
                'links' => [
                    'self' => route('api.muzibus.relationships.seo', $this->muzibu_id),
                    'related' => route('api.muzibus.seo', $this->muzibu_id)
                ],
                'data' => $this->when(
        ...
        if (in_array('activities', $includeRelations)) {
            $relationships['activities'] = [
                'links' => [
                    'self' => route('api.muzibus.relationships.activities', $this->muzibu_id),
                    'related' => route('api.muzibus.activities', $this->muzibu_id)
                ],
                'meta' => [
    ...
    private function getLinks(string $locale): array
    {
        ...
        return [
            'self' => route('api.muzibus.show', $this->muzibu_id),
            'frontend' => $slug ? route('muzibu.show', $slug) : null,
            'admin' => route('admin.muzibu.manage', $this->muzibu_id)
        ];
    }
    ...
    /**
     * Get available locales for this muzibu
     *
     * @return array<string>
     */
