...
class ES16_16WA_Istif_2_Detailed extends Seeder {
    public function run(): void {
        $p = DB::table('shop_products')->where('sku', 'ES16-16WA')->first();
        if (!$p) {$this->command->error('❌ Master bulunamadı: ES16-16WA'); return; }

        DB::table('shop_products')->where('product_id', $p->product_id)->update([
            'long_description' => json_encode(['tr' => '<section><h2>Operasyonların Ritmini Yükselten Akıllı İstif</h2>
<p>Günün ilk sevkiyatı kapıya yanaşırken, ekip raf yenilemeye çoktan başlar. İXTİF ES16-16WA dar koridorlarda sakin ama
emin adımlarla ilerler; AC sürüş sistemi yük altındayken bile akıcı hızlanma sağlar. 600&nbsp;mm yük merkezi ve
1600 kg nominal kapasite dengeli istifi mümkün kılar. İki kademeli indirme, üst raflara yaklaşırken
...
24V 230Ah (210 kg) yapılandırmaları desteklenir; entegre göstergede kalan süre takibi yapılabilir. Bakım erişim
...
<p>İş gücü verimliliği artık saniyelerle ölçülüyor. İXTİF ES16-16WA, güvenlik ve hız arasında ideal dengeyi kurarak toplam
...
JSON
        ...
        $this->command->info('✅ Detailed güncellendi: ES16-16WA');
    }
}
