...
namespace Modules\Service\App\Http\Resources;
 ...
 * Service Resource
 *
 * Transforms Service model for API responses.
 * Implements JSON API specification.
 *
 * @property \Modules\Service\App\Models\Service $resource
 */
class ServiceResource extends JsonResource
{
    ...
    private function getAttributes(string $locale, array $fields): array
    {
        $attributes = [
            ...
            'body' => $this->when(
                !isset($fields['services']) || in_array('body', $fields['services'] ?? []),
                fn() => $this->getTranslatedAttribute('body', $locale)
            ),
            'css' => $this->when(
                !isset($fields['services']) || in_array('css', $fields['services'] ?? []),
                $this->css
            ),
            'js' => $this->when(
                !isset($fields['services']) || in_array('js', $fields['services'] ?? []),
                $this->js
            ),
    ...
    private function getRelationships(array $includeRelations): array
    {
        ...
        if ($this->relationLoaded('seoSetting') || in_array('seo', $includeRelations)) {
            $relationships['seo'] = [
                'links' => [
                    'self' => route('api.services.relationships.seo', $this->service_id),
                    'related' => route('api.services.seo', $this->service_id)
                ],
                'data' => $this->when(
        ...
        if (in_array('activities', $includeRelations)) {
            $relationships['activities'] = [
                'links' => [
                    'self' => route('api.services.relationships.activities', $this->service_id),
                    'related' => route('api.services.activities', $this->service_id)
                ],
                'meta' => [
    ...
    private function getLinks(string $locale): array
    {
        ...
        return [
            'self' => route('api.services.show', $this->service_id),
            'frontend' => $slug ? route('service.show', $slug) : null,
            'admin' => route('admin.service.manage', $this->service_id)
        ];
    }
    ...
    /**
     * Get available locales for this service
     *
     * @return array<string>
     */
