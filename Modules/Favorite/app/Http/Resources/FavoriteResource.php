...
namespace Modules\Favorite\App\Http\Resources;
 ...
 * Favorite Resource
 *
 * Transforms Favorite model for API responses.
 * Implements JSON API specification.
 *
 * @property \Modules\Favorite\App\Models\Favorite $resource
 */
class FavoriteResource extends JsonResource
{
    ...
    private function getAttributes(string $locale, array $fields): array
    {
        $attributes = [
            ...
            'body' => $this->when(
                !isset($fields['favorites']) || in_array('body', $fields['favorites'] ?? []),
                fn() => $this->getTranslatedAttribute('body', $locale)
            ),
            'css' => $this->when(
                !isset($fields['favorites']) || in_array('css', $fields['favorites'] ?? []),
                $this->css
            ),
            'js' => $this->when(
                !isset($fields['favorites']) || in_array('js', $fields['favorites'] ?? []),
                $this->js
            ),
    ...
    private function getRelationships(array $includeRelations): array
    {
        ...
        if ($this->relationLoaded('seoSetting') || in_array('seo', $includeRelations)) {
            $relationships['seo'] = [
                'links' => [
                    'self' => route('api.favorites.relationships.seo', $this->favorite_id),
                    'related' => route('api.favorites.seo', $this->favorite_id)
                ],
                'data' => $this->when(
        ...
        if (in_array('activities', $includeRelations)) {
            $relationships['activities'] = [
                'links' => [
                    'self' => route('api.favorites.relationships.activities', $this->favorite_id),
                    'related' => route('api.favorites.activities', $this->favorite_id)
                ],
                'meta' => [
    ...
    private function getLinks(string $locale): array
    {
        ...
        return [
            'self' => route('api.favorites.show', $this->favorite_id),
            'frontend' => $slug ? route('favorite.show', $slug) : null,
            'admin' => route('admin.favorite.manage', $this->favorite_id)
        ];
    }
    ...
    /**
     * Get available locales for this favorite
     *
     * @return array<string>
     */
