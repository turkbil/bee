...
@section('module_content')
    @php
        ...
        $indexSlug = $moduleSlugService->getMultiLangSlug('ReviewSystem', 'index', $currentLocale);
        $showSlug = \App\Services\ModuleSlugService::getSlug('ReviewSystem', 'show');
        $defaultLocale = get_tenant_default_locale();
        $localePrefix = $currentLocale !== $defaultLocale ? '/' . $currentLocale : '';

        $breadcrumbsArray = [
            ['label' => __('reviewsystem::front.general.home'), 'url' => url('/')],
            ['label' => $moduleTitle ?? __('reviewsystem::front.general.reviewsystems')]
        ];
    @endphp
    ...
    <section class="bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        <div class="container mx-auto py-4">
            ...
            <h1 class="text-2xl md:text-3xl font-bold font-heading text-gray-900 dark:text-white">{{ $moduleTitle ?? __('reviewsystem::front.general.reviewsystems') }}</h1>
        </div>
    </section>
        ...
        <div class="container mx-auto">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    @foreach($items as $item)
                        @php
                            ...
                            $slug = $slug ?: $item->reviewsystem_id;
                            ...
                            <div class="p-5 md:p-6">
                                ...
                                <a href="{{ url($dynamicUrl) }}"
                                   class="inline-flex items-center gap-2 text-sm font-semibold text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors group/link">
                                    {{ __('reviewsystem::front.general.read_more') }}
            ...
            @else
                ...
                <div class="text-center py-16">
                    ...
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white mb-2 font-heading">{{ __('reviewsystem::front.general.no_reviewsystems_found') }}</h3>
                    <p class="text-gray-500 dark:text-gray-400 text-sm font-body">Henüz değerlendirme bulunmuyor.</p>
                </div>
            @endif
        </div>
    </section>
