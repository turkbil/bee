...
#[Layout('admin.layout')]
class ServiceCategoryManageComponent extends Component
{
    public $categoryId;
    ...
    public function currentCategory()
    {
        ...
        return ServiceCategory::query()->find($this->categoryId);
    }
    ...
    {
        $this->categoryService = app(\Modules\Service\App\Services\ServiceCategoryService::class);

        view()->share('pretitle', __('service::admin.category_management'));
        view()->share('title', __('service::admin.categories'));
    }
    ...
    protected function initializeUniversalComponents()
    {
        ...
        $this->tabConfig = \App\Services\GlobalTabService::getAllTabs('service_category');
        $this->activeTab = \App\Services\GlobalTabService::getDefaultTabKey('service_category');
    }
    ...
    public function handleLanguageChange($language)
    {
        if (in_array($language, $this->availableLanguages)) {
            ...
            Log::info('ServiceCategoryManage - Dil değişti', [
    ...
    protected function rules()
    {
        $rules = [
            ...
            'inputs.parent_id' => 'nullable|exists:service_categories,category_id',
        ];
    ...
    public function save($redirect = false, $resetForm = false)
    {
        ...
        if ($this->categoryId) {
            $category = ServiceCategory::query()->findOrFail($this->categoryId);
            $category->update($data);
            log_activity($category, 'güncellendi');

            $toast = [
                'title' => __('admin.success'),
                'message' => __('service::admin.category_updated'),
                'type' => 'success'
            ];
        } else {
            ...
            $toast = [
                'title' => __('admin.success'),
                'message' => __('service::admin.category_created'),
                'type' => 'success'
            ];
        }
    ...
    {
        return view('service::admin.livewire.category-manage-component', [
            'jsVariables' => [
    ...
    public function deleteCategory()
    {
        if (!$this->categoryId) {
            $this->dispatch('toast', [
                ...
                'message' => __('service::admin.category_not_found'),
                'type' => 'error'
            ]);
            return;
        }
