...
readonly class MuzibuRepository implements MuzibuRepositoryInterface
{
    ...
    public function __construct(
        private Muzibu $model
    ) {
        $this->cachePrefix = TenantCacheService::PREFIX_MUZIBU;
    ...
    public function findById(int $id): ?Muzibu
    {
        ...
        if (!$strategy->shouldCache()) {
            return $this->model->where('muzibu_id', $id)->first();
        }
        ...
        return $this->cache->remember(
            ...
            fn() => $this->model->where('muzibu_id', $id)->first()
        );
    }
    ...
    public function findByIdWithSeo(int $id): ?Muzibu
    {
        ...
        if ($strategy === CacheStrategy::ADMIN_FRESH) {
            return $this->model->with('seoSetting')->where('muzibu_id', $id)->first();
        }
        ...
        return $this->cache->remember(
            ...
            $strategy->getCacheTtl(),
            fn() => $this->model->with('seoSetting')->where('muzibu_id', $id)->first()
        );
    }
    ...
    public function getActive(): Collection
    {
        ...
        if (!$strategy->shouldCache()) {
            return $this->model->active()->orderBy('muzibu_id', 'desc')->get();
        }

        $cacheKey = $this->getCacheKey('active_muzibus');
        ...
        return Cache::tags($this->getCacheTags())
            ->remember(
                ...
                $strategy->getCacheTtl(),
                fn() =>
                $this->model->active()->orderBy('muzibu_id', 'desc')->get()
            );
    }
    ...
    public function getPaginated(array $filters = [], int $perPage = 10): LengthAwarePaginator
    {
        ...
        // Sorting
        $sortField = $filters['sortField'] ?? 'muzibu_id';
    ...
    {
        $result = $this->model->where('muzibu_id', $id)->update($data);
    ...
    {
        $result = $this->model->where('muzibu_id', $id)->delete();
    ...
    public function toggleActive(int $id): bool
    {
        ...
        $muzibu = $this->model->where('muzibu_id', $id)->first(['muzibu_id', 'is_active']);

        if (!$muzibu) {
            return false;
        }

        $result = $this->model->where('muzibu_id', $id)->update(['is_active' => !$muzibu->is_active]);
    ...
    {
        $count = $this->model->whereIn('muzibu_id', $ids)->delete();
    ...
    public function bulkToggleActive(array $ids): int
    {
        ...
        $muzibus = $this->model->whereIn('muzibu_id', $ids)->get(['muzibu_id', 'is_active']);
        $count = 0;

        foreach ($muzibus as $muzibu) {
            $this->model->where('muzibu_id', $muzibu->muzibu_id)
                ->update(['is_active' => !$muzibu->is_active]);
            $count++;
        }
    ...
    {
        $muzibu = $this->model->where('muzibu_id', $id)->first(['muzibu_id', 'seo']);
        ...
        $result = $this->model->where('muzibu_id', $id)->update(['seo' => $seo]);
