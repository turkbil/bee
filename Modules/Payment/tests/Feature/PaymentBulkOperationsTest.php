...
/**
 * Payment Bulk Operations Tests
 *
 * Toplu işlemlerin (bulk operations) doğru çalışmasını
 */
class PaymentBulkOperationsTest extends TestCase
{
    use RefreshDatabase;
    ...
    /** @test */
    public function it_can_bulk_delete_multiple_payments(): void
    {
    ...
    /** @test */
    public function it_can_bulk_toggle_payment_status(): void
    {
        $activePages = Payment::factory()->active()->count(3)->create();
        $inactivePages = Payment::factory()->inactive()->count(2)->create();

        $ids = $activePages->pluck('payment_id')->toArray();
        ...
        foreach ($activePages as $payment) {
            $this->assertFalse($payment->fresh()->is_active);
        }
    }

    /** @test */
    public function it_can_toggle_both_active_and_inactive_payments(): void
    {
        $mixedPages = collect([
        ...
        $ids = $mixedPages->pluck('payment_id')->toArray();
    ...
    {
        $payment = Payment::factory()->create();
        $payment->getOrCreateSeoSetting(); // SEO ayarı ekle

        $this->assertDatabaseHas('seo_settings', [
            'seo_settingable_id' => $payment->payment_id
        ]);

        $this->service->bulkDeletePages([$payment->payment_id]);
    ...
    public function bulk_operations_return_correct_result_objects(): void
    {
        ...
        $this->assertInstanceOf(\Modules\Payment\App\DataTransferObjects\BulkOperationResult::class, $result);
        $this->assertTrue($result->success);
    ...
    public function partial_bulk_delete_returns_partial_result(): void
    {
        ...
        $allIds = array_merge([$homepayment->payment_id], $payments->pluck('payment_id')->toArray());
    ...
    public function bulk_toggle_maintains_data_integrity(): void
    {
        ...
        foreach ($payments as $payment) {
            ...
            $this->assertEquals($payment->getTranslated('title', 'tr'), $fresh->getTranslated('title', 'tr'));
        }
    }
